#!/usr/bin/env python3
"""Run ET-C7 browser pressure with the tool-local Playwright runtime."""
...
def main() -> int:
    ...
    try:
        ...
        if result.returncode:
            raise RuntimeError("ET-C7 Playwright tests failed")
    finally:
        server.terminate()
        try:
            ...
            server.kill()
            server.wait(timeout=5)
    print("ET_C7_BROWSER_TEST_PASS projects=desktop,mobile tests=4 screenshots=6")
    return 0
