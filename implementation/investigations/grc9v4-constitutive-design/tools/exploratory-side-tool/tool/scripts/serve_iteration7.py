#!/usr/bin/env python3
"""Refresh ET-C7 state and serve the static review candidate locally."""
...
def main() -> int:
    ...
    build = subprocess.run(
        [sys.executable, str(TOOL_ROOT / "scripts/build_iteration7_ceilings.py")],
        cwd=repo_root,
        check=False,
    )
    if build.returncode:
    ...
    print(f"ET_C7_SERVE url=http://127.0.0.1:{args.port}", flush=True)
