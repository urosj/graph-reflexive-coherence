#!/usr/bin/env python3
"""Read-only scoped A_CI_PC research source, claim and debt queries."""
import argparse
import json
import sys
...
from grcv4_explorer import atc_cip
...
def main():
    ...
    if args.operation == "discover":
        value = atc_cip.observe_sources(root, atc_cip.pinned_admission(SIDE))
        print(json.dumps(value, indent=2))
        return 0 if value["state"] == "current_bundle_exact" else 2
    context = atc_cip.load_current_forensic_context(root, SIDE)
    if args.operation == "audit":
        ledger = context.documents_by_record[atc_cip.RECORD_ID].data
        value = dict(status="passed", accepted_research_claims=len(ledger["claims"]),
            profile_scope=["A_CI_PC"], debt_statuses=ledger["classifications"],
        ...
        value = (atc_cip.reconstruction_path if args.operation == "claim" else atc_cip.debt_lifecycle)(
