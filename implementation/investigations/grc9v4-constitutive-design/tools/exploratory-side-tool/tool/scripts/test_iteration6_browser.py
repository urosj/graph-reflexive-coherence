#!/usr/bin/env python3
"""Run I6 Playwright tests against a tool-local static preview server."""
...
def main() -> int:
    ...
    server = subprocess.Popen(
        [str(managed_node()), str(vite), "preview", "--host", "127.0.0.1", "--port", "4173", "--strictPort"],
        cwd=TOOL_ROOT / "web",
        env=environment,
    ...
    try:
        ...
        if result.returncode:
            raise RuntimeError("ET-C6 Playwright tests failed")
    finally:
        server.terminate()
        try:
            ...
            server.kill()
            server.wait(timeout=5)
    print("ET_C6_BROWSER_TEST_PASS projects=desktop,mobile screenshots=2")
    return 0
