#!/usr/bin/env python3
"""Run the ET-C11 D11 UX browser suite with historical regressions."""
...
def main() -> int:
    ...
    try:
        ...
        if result.returncode:
            raise RuntimeError("ET-C11 Playwright suite failed")
    finally:
        server.terminate()
        try:
            ...
            server.wait(timeout=5)
    print(
        "ET_C11_D11_BROWSER_TEST_PASS projects=desktop,mobile "
        "tests=16 screenshots=18 historical_regressions=12"
    )
    return 0
