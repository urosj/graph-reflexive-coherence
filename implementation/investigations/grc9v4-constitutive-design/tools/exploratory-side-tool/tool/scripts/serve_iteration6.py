#!/usr/bin/env python3
"""Refresh I6 discovery/build state and serve the static bundle locally."""
...
def main() -> int:
    ...
    build = subprocess.run(
        [sys.executable, str(TOOL_ROOT / "scripts/build_iteration6_navigation.py")],
        cwd=repo_root,
        check=False,
    )
    if build.returncode:
    ...
    print(f"ET_C6_SERVE url=http://127.0.0.1:{args.port}", flush=True)
    return subprocess.run(
        [str(managed_node()), str(vite), "preview", "--host", "127.0.0.1", "--port", str(args.port), "--strictPort"],
        cwd=TOOL_ROOT / "web",
