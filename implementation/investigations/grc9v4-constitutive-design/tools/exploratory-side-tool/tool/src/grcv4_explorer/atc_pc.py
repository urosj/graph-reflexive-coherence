"""Append-only paired A_PC research admission; never native ATC authority."""
...
EVIDENCE = atc.INV + "/evidence/atc-ci-pc"
LEDGER = atc.INV + "/evidence/atc-pc-admission/ATCPCClaimDebtLedger.json"
RECORD_ID = "GRCV4-ATC-PC-LEDGER-v1"
ADMISSION = "ATCPCResearchAdmission.json"
ADMISSION_DIGEST = "ee75994b958fbeb76aa7d39688bcce82510b9fccebf3990c062606dd2562584b"
CLAIMS = ("ATC-PC-STEP-01", "ATC-PC-CHAIN-01", "ATC-PC-CARRIER-03",
          "ATC-PC-CHAIN-02", "ATC-PC-DOMAIN-04")
_require, _path, _pointer = atc._require, atc._path, atc._pointer


def _inventory(root):
    return sorted([LEDGER] + [p.relative_to(root).as_posix()
...
def observe_sources(root, admission):
    ...
    value = dict(schema="grcv4_atc_pc_source_observation_v1", state=state,
...
def _validate_ledger(root, ledger):
    ...
    _require(contract["carrier_policy"] == "pc_star_fission_relational_lift_lossless_v1"
        and contract["native_authority"] is False, "PC selected contract changed")
    ...
    _require(tuple(c["claim_id"] for c in ledger["claims"]) == CLAIMS,
             "PC accepted claim roster changed")
    _require(len(debts) == len(ledger["debts"]) == 28
        and set(debts) == {d["id"] for d in origin["debts"]}, "PC debt roster changed")
    seen = set()
    for c in ledger["claims"]:
        _require(c["claim_class"] == "conditional" and c["profile_scope"] == ["A_PC"]
            and c["native_authority"] is False and c["status"] == "accepted_bounded_A_PC_research",
            "PC claim class/scope widened")
        row = _checked_ref(root, c["source_ref"])
        _require((row.get("family") == "A_PC" if c["claim_id"] == CLAIMS[0]
            else row.get("claim_id") == c["claim_id"] and row.get("profile_scope") == ["A_PC"]),
            "PC claim pointer mismatch")
        _require(set(c["predecessor_claim_ids"]) <= seen
            and set(c["origin_claim_ids"]) <= {o["id"] for o in origin["claims"]},
            "PC claim ancestry changed")
        seen.add(c["claim_id"])
        for did in c["debt_ids"]:
            _require(did in debts and c["claim_id"] in debts[did]["successor_claim_ids"],
                     "PC claim/debt reciprocity changed")
    for d in debts.values():
        ...
        _require(old["id"] == d["origin_debt_id"]
            ...
            "PC origin debt meaning/ceiling changed")
        _require(d["A_PC_research_status"] in ("closed", "partial", "not_activated")
            and d["transformation"] == "scoped_" + d["A_PC_research_status"]
            and d["admission_evidence"] == (d["origin_debt_id"] == "ATC7-DB-24"),
            "PC debt disposition changed")
        for cid in d["successor_claim_ids"]:
            _require(cid in claims and d["origin_debt_id"] in claims[cid]["debt_ids"],
                     "PC debt/claim reciprocity changed")
    _require(ledger["classifications"] == {s: sum(d["A_PC_research_status"] == s for d in debts.values())
        for s in ("closed", "partial", "not_activated")}, "PC debt counts changed")
    _require(ledger["unaccepted_handles"] == ["ATC-CI-STEP-01", "ATC-CI-CHAIN-01"],
             "PC admission widened to CI")
    _require(len(ledger["historical_controls"]) == 1, "PC control roster changed")
    control = ledger["historical_controls"][0]
    _require(control["claim_id"] == "ATC-PC-CARRIER-02" and control["selected_policy"] is False
        and control["status"] == "superseded_selection_explicit_loss_control"
        and control["successor_claim_id"] == "ATC-PC-CARRIER-03"
        and _checked_ref(root, control["source_ref"])["status"] == "projection_valid_selection_superseded",
        "PC control promoted to selected law")


def _build_context(root, side, admission):
    old = atc.load_current_forensic_context(root, side)
    _require(admission["historical_graph_digest"] == old.graph_digest
        ...
        "PC historical authority changed")
    observation = observe_sources(root, admission)
    _require(observation["state"] == "current_bundle_exact", "PC source set: " + observation["state"])
    ...
    for name in admission["evidence_files"]:
        ...
        if "record_digest" in data:
            _require(data["record_digest"] == record_digest(data, "record_digest"), "PC evidence digest drift")
        for ref in data.get("source_bindings", []):
            _require(file_sha256(_path(root, ref["path"])) == ref["sha256"],
                     "PC evidence binding drift: " + ref["path"])
    ...
    doc = SourceDocument(Path(LEDGER).name, root / LEDGER, ledger, row,
                         "atc_pc_research_v1", RECORD_ID, ledger["schema"], "record_digest", {})
    ...
    for i, d in enumerate(ledger["debts"]):
        key = node("scoped_debt_transformation", "A_PC:" + d["origin_debt_id"], f"/debts/{i}", d)
    ...
    _require(all(e[k] in overlay.nodes for e in overlay.propagation_edges.values() for k in ("source", "target")),
             "PC graph has unresolved endpoints")
    ...
    graph = dict(schema="grcv4_atc_pc_research_graph_v1", historical_graph_digest=old.graph_digest,
...
def pinned_admission(side):
    ...
    _require(value.get("schema") == "grcv4_atc_pc_research_admission_v1"
...
def load_current_forensic_context(root, side):
    ...
    _require(context.graph_digest == admission["graph_digest"], "PC graph no longer rebuilds exactly")
    return context


def reconstruction_path(context, claim_id):
    control = "historical_control:" + claim_id
    if control in context.nodes:
        n = context.nodes[control]
        return _trace(context, "reconstruction_path", {"claim_id": claim_id}, [_row(context,
            row_id=claim_id, classification="superseded_selection_explicit_loss_control",
            payload=n["attributes"], record_id=RECORD_ID, pointer=n["source_json_pointer"],
            edge_refs=_edges(context, lambda e:e["source"] == control))])
    start = "current_claim:" + claim_id
    if start not in context.nodes or context.nodes[start]["source_record_id"] != RECORD_ID:
        return atc.reconstruction_path(context, claim_id)
    reached = {start}
    while True:
    ...
    payload = dict(start_node=start, nodes=[context.nodes[n] for n in sorted(reached)],
        ...
        claim_class="conditional", support_disposition="accepted_bounded_A_PC_research",
        profile_scope=["A_PC"], native_authority=False, domain_widening_inferred=False,
...
def debt_lifecycle(context, debt_id):
    """PC-local disposition; the original debt and A_OS transformation stay separate."""
    key = "scoped_debt_transformation:A_PC:" + debt_id
    if key not in context.nodes:
        raise KeyError("unknown A_PC debt transformation: " + debt_id)
    n = context.nodes[key]
    return _trace(context, "debt_lifecycle", {"debt_id": debt_id, "profile_scope": "A_PC"}, [_row(context,
