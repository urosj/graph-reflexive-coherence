...
function accepted(extra={}) {
  return fixture({schema:'phase9_governance_status_v2',runtime_authorized:true,P9_G1_accepted:true,
    ...
    implementation_scope:Array.from({length:55},(_,i)=>({path:`synthetic/${i}`})),
...
test('accepted fixture readiness cannot imply a detector or open the final review', () => {
  const valid = accepted();
  checkedStatus(valid);
  assert.equal(valid.dependency_ready_leaves.length, 74);
...
test('full 7.1 permission requires every child without opening generic events or G2', () => {
  const valid = accepted();
...
test('scoped 7.2a permission retains every pair and unresolved initializer without support promotion', () => {
  const valid = accepted();
  ...
  assert.ok(leaves.includes('P9-7.2a-C_TO_A_UNRESOLVED'));
