...
test('accepted fixture readiness cannot imply a detector or open the final review', () => {
  const valid = accepted();
  checkedStatus(valid);
  assert.equal(valid.dependency_ready_leaves.length, 33);
...
test('A current/writer permission cannot imply later A work or A conformance', () => {
  const valid = accepted();
  checkedStatus(valid);
  for (const leaves of [valid.dependency_ready_leaves.filter(x => x !== 'P9-5.1'), valid.dependency_ready_leaves.filter(x => x !== 'P9-5.3'), [...valid.dependency_ready_leaves, 'P9-5.4']])
