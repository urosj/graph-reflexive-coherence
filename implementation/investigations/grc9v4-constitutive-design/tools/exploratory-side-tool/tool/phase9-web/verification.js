export function checkedStatus(value) {
  ...
  if (!implementation && value.P9_G1_accepted && (value.current_boundary !== 'failed_closed' || value.runtime_authority_state !== 'accepted_P9_G1_current_work_held' || value.implementation_scope !== undefined || value.permitted_runtime_paths !== undefined || value.dependency_ready_leaves !== undefined || value.foundation_acceptance !== undefined || value.request_acceptance !== undefined)) throw new Error('Held work cannot retain implementation permission');
  ...
  if (implementation && (JSON.stringify(value.dependency_ready_leaves) !== JSON.stringify(['P9-2.1','P9-2.2','P9-2.3','P9-2.4']) || !Array.isArray(value.permitted_runtime_paths) || value.permitted_runtime_paths.length !== 16 || !value.permitted_runtime_paths.includes('pyproject.toml') || value.permitted_runtime_paths.includes('src/pygrc/models/__init__.py'))) throw new Error('Invalid dependency-ready permission');
