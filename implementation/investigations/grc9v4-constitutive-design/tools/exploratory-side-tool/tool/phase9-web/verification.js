export function checkedStatus(value) {
  ...
  if (implementation && (JSON.stringify(value.dependency_ready_leaves) !== JSON.stringify(['P9-2.1','P9-2.2']) || !Array.isArray(value.permitted_runtime_paths) || value.permitted_runtime_paths.length !== 12 || !value.permitted_runtime_paths.includes('pyproject.toml') || value.permitted_runtime_paths.includes('src/pygrc/models/__init__.py'))) throw new Error('Invalid dependency-ready permission');
