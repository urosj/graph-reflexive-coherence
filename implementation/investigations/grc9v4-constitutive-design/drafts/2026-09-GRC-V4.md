...
**Status:** Draft — accepted P9 initializer design integrated from the accepted proposal; this paper revision awaits review. D11 content and earlier audit corrections are retained. Specification binding and implementation remain separate.
...
Candidate A also has a separately selected, history-free target initializer:
one target-only reference transport pass produces the current operand for the
unchanged full conductance law, followed by final realization-specific target
readmission. This resolves a design choice, not positive migration execution
or native formation.
...
The preclosure substrate-provenance obligation was completed for the D10/D11
80-object/183-contract population by D10.2 plus the two D11 provenance supplements.
Those source records left ten inherited and seven D11 forward obligations pending;
this revision does not re-audit later implementation follow-through or rewrite
their historical dispositions. The new initializer debt and still-forward
obligations are separately identified in §12.6.4 and Appendix G.5. Paper
integration does not satisfy specification propagation, implementation, runtime
...
## Appendix G. D11 and P9 Initializer Authority and Provenance Register

Sections G.1–G.4 retain the append-only D11 authority and its historical census.
Section G.5 separately records the P9 initializer successor. This appendix does
not reproduce or rewrite the immutable D10/D10.2 register or reclassify its claims.
...
Historical D10/D11 population accounting after both supplements is (later P9
additions are not counted or promoted by this census):
...
`D11-C-DEBT-BASELINE-TRANSPORT-AUTHORITY` and `D11-G9-DEBT-CANONICAL-PORT-ALLOCATION`
are resolved boundedly. The D11 source records retain their seventeen forward
obligations; the earlier D11 paper integration supplied the paper side of the
two paper-then-specification obligations. These historical dispositions do not
re-audit subsequent implementation or establish initializer migration evidence.

### G.5 P9 target-reference-pass initializer

The [accepted initializer design](../decisions/P9CandidateAInitializerReferencePassProposal.md)
and [structured authority](../decisions/P9CandidateAInitializerReferencePassAuthority.json)
are separately admitted P9 successors. This paper's integration follows the
accepted [proposal](GRCV4-proposal.md#126-graph-generic-candidate-a-history-free-initializer),
not a new source-rule selection made during paper editing.

| Source row | Paper location | Accepted meaning and limit |
| --- | --- | --- |
| `P9-7.2a-CL-O-INIT-001` at `/claim` | §§8.11, 12.6, 15.2; App. E.8 | Optional bounded construction, not inherited core, uniqueness or executed C→A |
| `P9-EC-A-INITIALIZER-REFERENCE-PASS` at `/contracts/0` | §12.6.1–12.6.3 | One target-only pass, full final conductance law, exact numerical staging, identity and final-admission boundary |
| `P9-O-A-INITIALIZER-REFERENCE-PASS` at `/object` | §§12.6, 14.4 | Graph-generic construction object; exact GRC9V3 initialization remains separate |
| `P9-7.2a-DEBT-A-INITIALIZER-SOURCE` at `/debt` | §12.6.4 | Producer choice resolved at bounded design scope |
| `P9-7.2a-VO-A-INITIALIZER-INTEGRATION` at `/verification_obligations/0` | §12.6.2–12.6.4 | Forward document/payload binding, implementation, five-realization evidence and aggregate review; not backward evidence |

The [forensic admission](../tools/exploratory-side-tool/records/P972aInitializerAdmission.json)
retains source/edge witnesses and separate claim, contract, object and debt
traces. The new contract has required support for this optional design;
`D10.2-EC-PARENT-L-A-INITIALIZER-GRC` retains its historical
`indeterminate_requires_review` association. Neither the old conditional role
nor supplied-flux construction evidence is retroactively labeled as this producer.

The reference-current rule is no longer undefined. This paper revision supplies
its proposed paper propagation, pending review; closed specification payloads,
release applicability, implementation and positive C→A through all five A
realizations remain forward work. No wider G2/G3, generic-event implementation,
native formation or automatic exact GRC9V3 binding follows. Historical release
and execution identities remain attached to their original sources, not these
