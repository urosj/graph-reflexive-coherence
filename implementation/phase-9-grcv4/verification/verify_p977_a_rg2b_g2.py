"""Bounded integrated A_RG2b G2 proposal; no acceptance or registry mutation.
...
import verify_p977_a_rg2b_crossings as crossing
import verify_p977_a_rg2b_acceptance as acceptance

SCRIPT = p.HERE + 'verify_p977_a_rg2b_g2.py'
TEST = p.HERE + 'test_p977_a_rg2b_g2.py'
INTERFACE = p.PHASE + 'tranche-7/P9-7.7-A_RG2b-G2Interface.json'
RECORD = p.PHASE + 'tranche-7/P9-7.7-A_RG2b-G2Review.json'
NOMINATED = crossing.NOMINATED
INTERFACE_TEST = 'test_p977_a_rg2b_g2.InterfaceTests.test_exact_profile_configuration_strict_run_and_missing_defaults'
OBLIGATIONS = ('G2-EXACT-PRODUCT', 'G2-ORDERED-ENDPOINTS', 'G2-INTEGRATED-REVIEW')
BOUNDED_COMMIT = '5ea4213'
...
def validate_interface(value):
    p.require(value['schema'] == 'phase9_a_rg2b_g2_interface_execution_v1'
    ...
    p.require(set(e['source_config'])=={'initial','differential_reference'}
              ...
              and e['source_config']['differential_reference']==before['differential_reference']
    ...
    p.require(before['scientific_state']['authoritative']['W_A']!=before['reset']['authoritative']['W_A']
              and before['scientific_state']['authoritative']['Z_4'] is None
              and before['reset']['authoritative']['Z_4'] is None, 'RG2b acquired carrier history or aliased W roles')
    ...
    expected = {'configuration_detached', 'immutable_parameters', 'unchanged_assignment', 'local_discovery_not_G2',
        ...
        'interrupted_iterator_keeps_prior_commit', 'W_configuration_detached', 'reset_W_preserved',
        'no_carrier_slot', 'frozen_completion_unchanged'}
...
def capture_interface():
    from test_p977_a_rg2b_g2 import InterfaceTests
    ...
    value = dict(schema='phase9_a_rg2b_g2_interface_execution_v1', captured_at=datetime.now(timezone.utc).isoformat(),
...
def authority():
    ...
    contracts = ('D10.2-EC-PARENT-REAL-RG2B', 'D10.2-EC-RG-INVARIANCE',
...
def build(bounded_acceptance=None):
    from a_rg2b_g2_source_reuse import retained_bindings
    ...
    product = []
    for case in sorted(review.required_cases('A_RG2b')):
    ...
    initializer_target = next(r['target_active_model_identity'] for r in b['retained_aliases'] if r['key'] == 'initializer/A_RG2b')
    ...
    sources.update({n: p.sha((p.ROOT/n).read_bytes()) for n in (INTERFACE, crossing.RECORD,
        acceptance.REVIEW, p.HERE+'verify_p977_a_rg2b_acceptance.py', 'specs/grc-common-interface.md',
    ...
    value = dict(schema='phase9_a_rg2b_integrated_g2_review_v1', gate='P9-G2[A_RG2b]', verdict='PASS_PROPOSAL',
        ...
        ordered_scope=dict(matrix=b['ordered_migration_matrix'], C_to_nominated_A='rejected_unselected_initializer',
            ...
            retained_scalar_RG2b_is_not_graph_nomination=True,
            graph_companion_declarations_are_not_proposed_support=True,
            C_to_selected_initializer='separate_retained_scalar_target_not_nomination_execution',
            A_to_C='exact_graph_nomination_source_with_candidate_history_loss',
            event_policy='separate_selected_initializer_target_with_explicit_W_loss_no_carrier',
            event_completion_is_separate_and_frozen=True,
        ...
        numerical_tests_rerun=0, reused_local_methods=4, reused_crossing_methods=3,
...
def validate(value, expected):
    ...
    p.require([r['fixture_id'] for r in value['catalog_product']] == sorted(review.required_cases('A_RG2b')),
