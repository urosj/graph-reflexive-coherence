"""Narrow C_CI_PC facade supplement and read-only integrated-review pressure."""
...
class InterfaceTests(unittest.TestCase):
    def test_exact_profile_configuration_strict_run_and_missing_defaults(self):
        ...
        inputs, backend = fixture('C_CI_PC')
        ...
        config['initial']['current']['Z_4'][0] = 999
        config['initial']['reset']['Z_4'][0] = -999
        ...
        with self.assertRaises(FrozenInstanceError):
            owner.get_params().realization.tau_PC = 999
        self.assertEqual(owner.get_params(), inputs.geometry.reference.profile.params_resolved)
        ...
        self.assertNotIn(NOMINATED, list_supported_profiles())  # Local construction is not public G2 support.
        ...
        self.assertNotEqual(initial['scientific_state']['authoritative']['Z_4'],initial['reset']['authoritative']['Z_4'])
        ...
        self.__class__.evidence = dict(source_config=source, initial_snapshot=initial,
            ...
            checks=dict(configuration_detached=True, immutable_parameters=True, unchanged_assignment=True,
                ...
                strict_external_results_equal=True, interrupted_iterator_keeps_prior_commit=True, carrier_configuration_detached=True, carrier_reset_preserved=True),
            operation_scope='zero_duration_interface_only; positive numerical step/replay remains in local product')


class ReviewTests(unittest.TestCase):
    def test_rehashed_interface_evidence_rejects_missing_or_widened_checks(self):
        import verify_p977_c_ci_pc_g2 as gate
        original=review.read(gate.INTERFACE)
        gate.validate_interface(original)
        edits={
            ...
            'carrier_alias':lambda v:v['evidence']['checks'].update(carrier_configuration_detached=False),
            'missing_reset':lambda v:v['evidence']['checks'].pop('carrier_reset_preserved'),
            'positive_dt':lambda v:v['evidence']['strict_request'].update(dt=.125),
            'reset_history':lambda v:v['evidence']['after_stream']['reset']['authoritative'].update(Z_4=[0.]),
        ...
        for name,edit in edits.items():
            with self.subTest(name=name):
                ...
                with self.assertRaises((ValueError,KeyError)):gate.validate_interface(bad)
    ...
    def test_integrated_product_and_rehashed_false_proposals(self):
        ...
        edits = {
            ...
            'family_alias': lambda v: v.update(proposed_additional_support=['C_CI_PC']),
            ...
            'unselected_initializer': lambda v: v['ordered_scope'].update(C_to_A='any_A_target'),
            'borrowed_event': lambda v: v.update(proposed_additional_support=[v['ordered_scope']['event_target']]),
            'carrier_reset_as_release': lambda v:v['ordered_scope'].update(event_policy='native_release'),
            'matched_forcing_overclaim': lambda v:v['ordered_scope'].update(matched_forcing_contraction_claimed=True),
            'base_chart_overclaim': lambda v:v['ordered_scope'].update(indefinite_base_chart_invariance_claimed=True),
            'PC_pair_widening': lambda v:v['ordered_scope'].update(exact_PC_pairs_preserve_carrier_only_under_exact_carrier_contract=False),
            'invented_W': lambda v:v['ordered_scope'].update(C_has_no_W_state_or_A_writer=False),
            'baseline_authority': lambda v:v['ordered_scope'].update(full_C_baseline_rederived_at_each_role=False),
            'initializer_loss': lambda v:v['ordered_scope'].update(C_to_A_candidate_history_loss=True),
            'coupled_source_split':lambda v:v['ordered_scope'].update(same_selected_root_source_drives_geometry_and_ZOH=False),
            'history_written_in_trial':lambda v:v['ordered_scope'].update(trial_history_frozen=False),
            'global_root':lambda v:v['ordered_scope'].update(global_root_uniqueness_claimed=True),
            'settled_gain_two':lambda v:v['ordered_scope'].update(gain_two_is_constant_source_law_identity_not_settled_trajectory=False),
            'amplitude_equivalence':lambda v:v['ordered_scope'].update(unit_plus_unit_gain_is_not_amplitude_equivalence=False),
            'zero_carrier_flattening':lambda v:v['ordered_scope'].update(zero_event_carrier_does_not_erase_immediate_geometry=False),
            'A_to_C_loss_erased':lambda v:v['ordered_scope'].update(A_to_C_candidate_history_loss=False),
            'invented_C_W_writer':lambda v:v['ordered_scope'].update(separate_post_continuity_W_writer=True),
