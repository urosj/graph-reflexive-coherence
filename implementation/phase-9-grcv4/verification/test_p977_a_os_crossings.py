"""Exact A_OS crossing gaps; retained migrations are projected, not rerun."""
...
class AOSCrossingTests(unittest.TestCase):
    ...
    def test_persistent_crossings_preserve_W_and_account_for_Z_independently(self):
        for source, target_family in (('A_OS', 'A_PC'), ('A_PC', 'A_OS')):
            ...
            for a, b in ((old.current, new.current), (old.reset.authoritative, new.reset.authoritative)):
                self.assertEqual((b.C, b.W_A), (a.C, a.W_A))
                self.assertEqual(b.Z_4, (0.,) if target_family == 'A_PC' else None)
            history = result.emitted_receipts[0].identity_payload['history']
            self.assertEqual(history['candidate']['disposition'], 'exact_transport')
            self.assertEqual(history['carrier']['disposition'], 'target_initializer' if source == 'A_OS' else 'explicit_loss')
            losses = [] if source == 'A_OS' else ['carrier_history_loss']
            ...
            # Bind the migration to its immediate poststate, not the subsequent reset.
            migrated = restore(after)
            self.emit('persistent_' + source, before, migrated, result, declared,
                after_reset_object=reset_object, expected_losses=losses,
                expected_candidate='exact_transport', expected_carrier=history['carrier']['disposition'])

    def test_mapped_event_distinct_roles_initializer_oracle_and_reset(self):
        inputs, backend = fixture('A_OS')
        target, other = initializer_target('A_OS')
        ...
        before, old = owner.snapshot(), owner.state.lifecycle
        declared = event(owner, target, matrix=(0, 1, 1, 0), increment=(.125, 0), operation='A_OS-mapped-event')
    ...
    def test_reset_only_readmission_and_unavailable_incoming_initializer(self):
        ...
        selected, _ = initializer_target('A_OS')
