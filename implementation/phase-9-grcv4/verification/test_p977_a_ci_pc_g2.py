"""Narrow A_CI_PC facade supplement and read-only integrated-review pressure."""
...
class InterfaceTests(unittest.TestCase):
    def test_exact_profile_configuration_strict_run_and_missing_defaults(self):
        ...
        self.assertIn(NOMINATED, list_supported_profiles())  # Exact public G2 acceptance; local discovery alone grants none.
...
class ReviewTests(unittest.TestCase):
    def test_rehashed_interface_evidence_rejects_missing_or_widened_checks(self):
        import verify_p977_a_ci_pc_g2 as gate
    ...
    def test_integrated_product_and_rehashed_false_proposals(self):
        ...
        edits = {
            ...
            'family_alias': lambda v: v.update(proposed_additional_support=['A_CI_PC']),
            ...
            'PC_pair_widening': lambda v:v['ordered_scope'].update(exact_PC_pairs_preserve_both_histories_only_under_exact_carrier_contract=False),
            'formation_overclaim': lambda v:v['ordered_scope'].update(supplied_W_is_not_authenticated_formation=False),
            'PC_only_source_scope':lambda v:v['ordered_scope'].update(CI_PC_scope='old_Z_only'),
            'second_source':lambda v:v['ordered_scope'].update(same_selected_root_source_drives_geometry_and_ZOH=False),
            'global_root_claim':lambda v:v['ordered_scope'].update(global_root_uniqueness_claimed=True),
            'steady_trajectory_claim':lambda v:v['ordered_scope'].update(gain_two_is_constant_source_law_identity_not_settled_trajectory=False),
            'borrowed_event_support':lambda v:v.update(proposed_additional_support=[v['ordered_scope']['event_target']]),
            'collapsed_initializer_targets':lambda v:v['ordered_scope'].update(event_target=v['ordered_scope']['initializer_target']),
