"""Retained A_CI_PC crossing pressure; no lifecycle reruns."""
...
class CrossingEvidenceTests(unittest.TestCase):
    def test_rehashed_scope_map_history_and_endpoint_mutations(self):
        ...
        mutations={
            ...
            'borrowed_source':lambda v:row(v,'mapped_event').update(complete_profile_id='A_CI_PC'),
            ...
            'event_fixed_reference_geometry':lambda v:event(v)['expectations']['current'].update(h=[[1.]]),
            'event_immediate_source_off':lambda v:event(v)['expectations']['reset'].update(source=[[0.]]),
            ...
            'incoming_W_changed':lambda v:row(v,'nonpersistent_incoming')['observation']['expectations']['reset'].update(W_A=[999.]),
            'outgoing_Z_retained':lambda v:row(v,'nonpersistent_outgoing')['observation']['expectations']['current'].update(Z_4=[.125]),
            'incoming_root_changed':lambda v:row(v,'nonpersistent_incoming')['observation']['expectations']['current'].update(current=[999.]),
            ...
            'lost_replay':lambda v:row(v,'nonpersistent_incoming')['observation'].update(exact_replay=False),
