"""Exact A_OS crossing reconciliation. --run emits a fresh record to stdout."""
...
SCRIPT = p.HERE + 'verify_p977_a_os_crossings.py'
TEST = p.HERE + 'test_p977_a_os_crossings.py'
RECORD = p.PHASE + 'tranche-7/P9-7.7-A_OS-Crossings.json'
NOMINATED = 'grcv4-profile-sha256:e4c04a83240a33d77c50a26ca6effbb6ce142774bd01f0966861dd517a94e2c4'
CASES = ['incoming_C_rejection', 'mapped_event', 'persistent_A_OS', 'persistent_A_PC',
...
def roster():
    from test_p977_a_os_crossings import AOSCrossingTests
    return ['test_p977_a_os_crossings.AOSCrossingTests.' + n
            for n in unittest.TestLoader().getTestCaseNames(AOSCrossingTests)]


def retained():
    """Project only fields actually present in the old receipt/endpoints."""
    aliases = []
    for origin, key in [('migration', k) for k in ('A_NH_NH', 'A_C_NH', 'A_PC_CIPC', 'A_CIPC_PC')] + [('initializer', 'A_OS')]:
...
def matrix():
    return [
        dict(migration_class='same_candidate_nonhistory_to_nonhistory', disposition='positive_exact_nomination_source', evidence='migration/A_NH_NH'),
        dict(migration_class='same_candidate_nonhistory_to_history', disposition='positive_exact_nomination_source', evidence='persistent_A_OS'),
        ...
        dict(migration_class='PC_to_CI_PC', disposition='separate_endpoints_not_A_OS; no positive nomination credit', evidence='migration/A_PC_CIPC'),
        dict(migration_class='CI_PC_to_PC', disposition='separate_endpoints_not_A_OS; no positive nomination credit', evidence='migration/A_CIPC_PC'),
        dict(migration_class='A_to_C', disposition='positive_exact_nomination_source', evidence='migration/A_C_NH'),
        dict(migration_class='C_to_A', disposition='negative_exact_nomination_target; positive initializer target remains a different profile',
             evidence='incoming_C_rejection', separate_positive='initializer/A_OS'),
    ]


def cells():
    return {
        'A-MIGRATION-HISTORY-RECEIPT': ['migration/A_NH_NH', 'migration/A_C_NH', 'persistent_A_OS', 'persistent_A_PC'],
        'RESET-AFTER-MIGRATION': ['migration/A_NH_NH', 'migration/A_C_NH', 'persistent_A_OS', 'persistent_A_PC'],
        'RESET-AFTER-EVENT': ['mapped_event'],
        'WHOLE-LIFECYCLE-TUPLE-MAP': ['mapped_event'],
        'ALL-MIGRATION-CLASSES': ['ordered_migration_matrix'],
        'HISTORY-DISPOSITION': ['migration/A_C_NH', 'persistent_A_OS', 'persistent_A_PC', 'mapped_event'],
...
def validate(value):
    ...
    p.require(set(cells()) == set(review.required_cases('A_OS')) - set(local.required()), 'crossing catalog drift')
    ...
    for row in rows:
        ...
        case = row['fixture_id']
        if case in ('persistent_A_OS', 'persistent_A_PC', 'mapped_event'):
        ...
        if case.startswith('persistent_'):
            p.require(NOMINATED == (row['complete_profile_id'] if case == 'persistent_A_OS' else request['target_profile_id']), 'wrong persistent endpoint')
            for left, right in ((before['scientific_state']['authoritative'], after['scientific_state']['authoritative']),
                ...
                p.require(left['C'] == right['C'] and left['W_A'] == right['W_A']
                          and right['Z_4'] == ([0] if case == 'persistent_A_OS' else None), 'wrong literal migration map')
            ...
            p.require(not row['committed'] and request['target_profile_id'] == NOMINATED
                      and obs['selected_initializer_profile_id'] != NOMINATED, 'incoming negative mislabeled')
        else:
...
def run():
    ...
    value = dict(schema='phase9_exact_profile_crossing_reconciliation_v1', iteration_id='P9-7.7-A_OS-crossings',
        ...
        fixture_results=AOSCrossingTests.rows, objects=AOSCrossingTests.objects,
        retained_aliases=retained(), ordered_migration_matrix=matrix(), catalog_cells=cells(),
