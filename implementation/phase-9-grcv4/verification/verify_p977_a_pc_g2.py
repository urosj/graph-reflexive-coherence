"""Bounded integrated A_PC G2 proposal; no acceptance or registry mutation.
...
import verify_p977_a_pc_acceptance as acceptance

SCRIPT = p.HERE + 'verify_p977_a_pc_g2.py'
TEST = p.HERE + 'test_p977_a_pc_g2.py'
INTERFACE = p.PHASE + 'tranche-7/P9-7.7-A_PC-G2Interface.json'
RECORD = p.PHASE + 'tranche-7/P9-7.7-A_PC-G2Review.json'
NOMINATED = crossing.NOMINATED
INTERFACE_TEST = 'test_p977_a_pc_g2.InterfaceTests.test_exact_profile_configuration_strict_run_and_missing_defaults'
...
def validate_interface(value):
    p.require(value['schema'] == 'phase9_a_pc_g2_interface_execution_v1'
...
def capture_interface():
    from test_p977_a_pc_g2 import InterfaceTests
    ...
    value = dict(schema='phase9_a_pc_g2_interface_execution_v1', captured_at=datetime.now(timezone.utc).isoformat(),
...
def authority():
    ...
    contracts = ('D10.2-EC-PARENT-REAL-PC', 'D10.2-EC-PC-ZOH-WRITER', 'D10.2-EC-PC-RELEASE', 'D10.2-EC-PC-MATCHED-FORCING', 'P9-EC-A-INITIALIZER-REFERENCE-PASS', 'P9-EC-RECEIPT-PARENT-CHAIN',
...
def build(bounded_acceptance=None):
    ...
    product = []
    for case in sorted(review.required_cases('A_PC')):
    ...
    initializer_target = next(r['target_active_model_identity'] for r in b['retained_aliases'] if r['key'] == 'initializer/A_PC')
    ...
    sources.update({n: p.sha((p.ROOT/n).read_bytes()) for n in (INTERFACE, crossing.RECORD,
        acceptance.REVIEW, p.HERE+'verify_p977_a_pc_acceptance.py', 'specs/grc-common-interface.md',
    ...
    value = dict(schema='phase9_a_pc_integrated_g2_review_v1', gate='P9-G2[A_PC]', verdict='PASS_PROPOSAL',
        ...
        ordered_scope=dict(matrix=b['ordered_migration_matrix'], C_to_nominated_A='rejected_unselected_initializer',
            ...
            PC_scope='old_committed_Z_read_one_held_source_ZOH_write_then_both_role_readmission',
...
def validate(value, expected):
    ...
    p.require([r['fixture_id'] for r in value['catalog_product']] == sorted(review.required_cases('A_PC')),
