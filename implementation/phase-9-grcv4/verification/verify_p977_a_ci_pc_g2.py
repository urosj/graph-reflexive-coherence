"""Bounded integrated A_CI_PC G2 proposal; no acceptance or registry mutation.
...
import verify_p977_a_ci_pc_crossings as crossing
import verify_p977_a_ci_pc_acceptance as acceptance

SCRIPT = p.HERE + 'verify_p977_a_ci_pc_g2.py'
TEST = p.HERE + 'test_p977_a_ci_pc_g2.py'
INTERFACE = p.PHASE + 'tranche-7/P9-7.7-A_CI_PC-G2Interface.json'
RECORD = p.PHASE + 'tranche-7/P9-7.7-A_CI_PC-G2Review.json'
NOMINATED = crossing.NOMINATED
INTERFACE_TEST = 'test_p977_a_ci_pc_g2.InterfaceTests.test_exact_profile_configuration_strict_run_and_missing_defaults'
...
def validate_interface(value):
    p.require(value['schema'] == 'phase9_a_ci_pc_g2_interface_execution_v1'
...
def capture_interface():
    from test_p977_a_ci_pc_g2 import InterfaceTests
    ...
    value = dict(schema='phase9_a_ci_pc_g2_interface_execution_v1', captured_at=datetime.now(timezone.utc).isoformat(),
...
def authority():
    ...
    contracts = ('D10.2-EC-PARENT-REAL-CI-PC', 'D10.2-EC-CI-PC-A-COMPOSITION',
        'D10.2-EC-CI-PC-A-ROOT', 'D10.2-EC-CI-A-CONTRACTION', 'D10.2-EC-CI-PC-ABLATIONS', 'D10.2-EC-PC-ZOH-WRITER', 'D10.2-EC-PC-RELEASE', 'D10.2-EC-PC-MATCHED-FORCING', 'P9-EC-A-INITIALIZER-REFERENCE-PASS', 'P9-EC-RECEIPT-PARENT-CHAIN',
...
def build(bounded_acceptance=None):
    from a_ci_pc_g2_source_reuse import retained_bindings
    ...
    product = []
    for case in sorted(review.required_cases('A_CI_PC')):
    ...
    initializer_target = next(r['target_active_model_identity'] for r in b['retained_aliases'] if r['key'] == 'initializer/A_CI_PC')
    ...
    p.require(event_target!=initializer_target and event_target!=NOMINATED,'separate event endpoint collapsed')
    p.git(p.ROOT,'merge-base','--is-ancestor','d41e1c6','HEAD')
    for path in (local.RECORD,crossing.RECORD,acceptance.REVIEW):
        p.require(p.git(p.ROOT,'show','d41e1c6:'+path)==(p.ROOT/path).read_bytes(),'bounded checkpoint changed')
    ...
    sources.update({n: p.sha((p.ROOT/n).read_bytes()) for n in (INTERFACE, crossing.RECORD,
        acceptance.REVIEW, p.HERE+'verify_p977_a_ci_pc_acceptance.py', 'specs/grc-common-interface.md',
        'specs/grc-v4-a-initializer-spec.md', 'specs/grc-v4-topology-event-spec.md')})
    value = dict(schema='phase9_a_ci_pc_integrated_g2_review_v1', gate='P9-G2[A_CI_PC]', verdict='PASS_PROPOSAL',
        ...
        bounded_acceptance=dict(commit='d41e1c6',path=acceptance.REVIEW, sha256=acceptance.ACCEPTANCE_SHA256, execution_digest=b['record_digest']),
        ...
        ordered_scope=dict(matrix=b['ordered_migration_matrix'], C_to_nominated_A='rejected_unselected_initializer',
            ...
            event_target_differs_from_retained_initializer=True,
            ...
            exact_PC_pairs_preserve_both_histories_only_under_exact_carrier_contract=True,
            ...
            event_policy='explicit_candidate_loss_and_whole_carrier_reset_not_native_release',
            CI_PC_scope='old_C_W_Z_joint_root_same_selected_source_for_immediate_geometry_and_one_ZOH_then_both_role_readmission',
            same_selected_root_source_drives_geometry_and_ZOH=True,
            separate_post_continuity_W_writer=True,trial_history_frozen=True,
            ...
            supplied_W_is_not_authenticated_formation=True,
...
def validate(value, expected):
    ...
    p.require([r['fixture_id'] for r in value['catalog_product']] == sorted(review.required_cases('A_CI_PC')),
