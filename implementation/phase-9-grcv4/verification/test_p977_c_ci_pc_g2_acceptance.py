"""Exact C_CI_PC decision, unchanged evidence, and finite source-reuse pressure."""
...
import c_ci_pc_g2_source_reuse as reuse
...
class AcceptanceTests(unittest.TestCase):
    def test_exact_discovery_and_no_control_or_target_promotion(self):
        ...
        row = next(r for r in g.registry(p.ROOT)['records'] if r['profile_family_id']=='C_CI_PC')
        self.assertEqual(row['profile_family_id'], 'C_CI_PC')
        ...
        self.assertEqual(len(list_supported_profiles()), 8)
    ...
    def test_review_runs_and_prior_acceptances_unchanged(self):
        ...
        names = [prefix+'C_CI_PC-'+suffix for suffix in (
            'G2Review.json', 'G2Review.md', 'G2Interface.json', 'LocalProduct.json', 'Crossings.json')]
        names += [prefix+family+'-'+suffix for family in ('A_CI', 'A_OS', 'C_CI', 'A_PC', 'C_PC', 'A_CI_PC')
    ...
    def test_current_integrated_review_projects_exact_acceptance(self):
        import verify_p977_c_ci_pc_g2 as gate
        bounded = gate.acceptance.check(local_product={
            'record_digest': gate.review.read(gate.local.RECORD)['record_digest']})
        raw = gate.check(bounded_acceptance=bounded)
        row = next(r for r in g.registry(p.ROOT)['records'] if r['profile_family_id']=='C_CI_PC')
        support = g.checked(p.ROOT)['accepted_generic_runtime_support']
        projected = g.project_review(p.ROOT, row, raw, support)
        self.assertEqual(projected['status'], 'accepted')
        self.assertTrue(projected['G2_accepted'])
        self.assertEqual(projected['catalog_cells'], 33)
        self.assertEqual(projected['new_G2_support'], [gate.NOMINATED])
        self.assertFalse(projected['all_ordered_pairs_verified'])
