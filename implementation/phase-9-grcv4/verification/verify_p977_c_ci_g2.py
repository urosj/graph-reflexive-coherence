"""Bounded integrated C_CI G2 proposal; no acceptance or registry mutation.
...
SCRIPT = p.HERE + 'verify_p977_c_ci_g2.py'
TEST = p.HERE + 'test_p977_c_ci_g2.py'
INTERFACE = p.PHASE + 'tranche-7/P9-7.7-C_CI-G2Interface.json'
RECORD = p.PHASE + 'tranche-7/P9-7.7-C_CI-G2Review.json'
NOMINATED = crossing.NOMINATED
INTERFACE_TEST = 'test_p977_c_ci_g2.InterfaceTests.test_exact_profile_configuration_strict_run_and_missing_defaults'
...
def validate_interface(value):
    p.require(value['schema'] == 'phase9_c_ci_g2_interface_execution_v1'
...
def capture_interface():
    from test_p977_c_ci_g2 import InterfaceTests
    ...
    value = dict(schema='phase9_c_ci_g2_interface_execution_v1', captured_at=datetime.now(timezone.utc).isoformat(),
...
def authority():
    ...
    contracts = ('D10.2-EC-CI-C-ROOT', 'D10.2-EC-CI-C-CONTRACTION', 'D10.2-EC-CI-C-ROOT-SELECTION',
        'D11-C-EC-C-TR-REFERENCE-FIELD', 'D11-C-EC-C-M4-FACTORIZATION', 'D11-C-EC-C-J0-CURRENT',
        'D11-C-EC-C-J0-DERIVATIVE', 'D11-C-EC-C-J0-COVARIANCE', 'D11-C-EC-C-J0-LIFECYCLE',
        'D11-C-EC-C-J0-REALIZATION-COVERAGE', 'P9-EC-A-INITIALIZER-REFERENCE-PASS', 'P9-EC-RECEIPT-PARENT-CHAIN',
...
def build(bounded_acceptance=None):
    ...
    product = []
    for case in sorted(review.required_cases('C_CI')):
    ...
    initializer_target = next(b['objects'][r['request_object']]['target_profile_id'] for r in b['fixture_results'] if r['fixture_id']=='C_to_A')
    event_target = next(b['objects'][r['request_object']]['target_profile_id'] for r in b['fixture_results'] if r['fixture_id']=='mapped_event')
    # Bootstrap the proposal before registration; once registered, preserve its
    # ordered predecessor set even after later profile acceptances.
    prior = support_before(p.ROOT,NOMINATED) if any(r['complete_profile_id']==NOMINATED for r in registry(p.ROOT)['records']) else sorted(list_supported_profiles())
    sources = interface_bindings()
    ...
    value = dict(schema='phase9_c_ci_integrated_g2_review_v1', gate='P9-G2[C_CI]', verdict='PASS_PROPOSAL',
        ...
        ordered_scope=dict(matrix=b['ordered_migration_matrix'], C_to_A='positive_nomination_source_to_selected_initializer_target',
            ...
            nominal_kappa_M_C=0, dense_controls_are_separate_profiles=True,
            CI_scope='bounded_local_reference_ball_not_global_uniqueness',
            all_ordered_pairs_verified=False, endpoint_support_does_not_prove_crossing_support=True,
            separate_PC_pairs_not_C_CI_credit=True,
...
def validate(value, expected):
    ...
    p.require([r['fixture_id'] for r in value['catalog_product']] == sorted(review.required_cases('C_CI')),
