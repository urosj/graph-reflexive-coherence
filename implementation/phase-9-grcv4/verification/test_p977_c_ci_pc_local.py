"""Exact C_CI_PC local product: complete trial baseline, coupled same-root geometry/ZOH, no W writer, no G2."""
...
from tests.models.test_grc_v4_cipc import independent_root as coupled_root, configure as coupled_configure
...
def independent_beat(inputs):
    h,j,source=coupled_root(inputs,None)
...
class CCIPCLocalProductTests(unittest.TestCase):
    obj = catalog_helpers.CompleteProfileCatalogTests.obj

    @classmethod
    def setUpClass(cls):
        cls.rows, cls.objects, cls.pressure = [], {}, []
        cls.inputs, cls.backend = fixture('C_CI_PC')
    ...
    def test_coupled_root_complete_trial_and_role_reconstruction(self):
        ...
        for label,inputs in (('nomination_seed',self.inputs),('signed_history_companion',history)):
            expected=independent_beat(inputs);trials=[]
            native=ci._point
            def observe(subject,backend):
                point=native(subject,backend)
                if subject.current==inputs.current and subject.stage=='cipc_trial':
                    trials.append(point)
                return point
            with patch.object(ci,'_point',side_effect=observe), \
                 patch.object(ci,'CandidateAWriter',wraps=ci.CandidateAWriter) as writers, \
                 patch.object(pc,'scalar_zoh',wraps=pc.scalar_zoh) as carriers:
                value=ci.ProvisionalCandidateCIStep(inputs)
            root=value.root;selected=root.selected
            ...
            self.assertEqual(selected.inputs.stage,'cipc_trial')
            self.assertEqual(selected.inputs.current,inputs.current)
            self.assertEqual(value.reset_root.inputs.current,inputs.reset)
            ...
            self.assertEqual(carriers.call_args.args[1],tuple(source.flat))
            self.assertFalse(np.allclose(source,value.restart.selected.structural_source.increment,rtol=0,atol=1e-10))
            observed=dict(h=np.asarray(selected.inputs.geometry.one_form_hodge.matrix).tolist(),
                current=list(root.current.values),source=source.tolist(),
                final_C=list(value.next_inputs.current.C),written_Z=list(value.next_inputs.current.Z_4))
            for key in observed:
                np.testing.assert_allclose(observed[key],expected[key],rtol=0,atol=2e-11 if key in ('h','current','source') else 2e-13)
            np.testing.assert_allclose(observed['h'],old_h(inputs)+inputs.geometry.reference.profile.params_resolved.geometry.kappa_H*source,rtol=0,atol=2e-11)
            self.assertGreater(np.linalg.norm(np.asarray(observed['h'])-old_h(inputs)),1e-8)
            self.assertGreater(len(trials),1)
            trial_rows=[]
            for point in trials:
                self.assertEqual(point.inputs.current,inputs.current)
                native_chain=observed_chain(point);oracle=dense_current_oracle(point.inputs)
                for key in native_chain:np.testing.assert_allclose(native_chain[key],oracle[key],rtol=0,atol=2e-12)
                trial_rows.append(dict(inputs_object=self.obj(point.inputs.to_payload()),observed=native_chain,
                    independent_oracle={k:oracle[k].tolist() for k in native_chain}))
            self.assertNotEqual(trials[0].algebra.baseline.values,selected.point.algebra.baseline.values)
            chains=[]
            for role,admitted in (('consumed',root),('reset',value.reset_root),('restart',value.restart)):
                native_chain=observed_chain(admitted.selected.point)
                oracle=dense_current_oracle(admitted.selected.inputs)
                rh,rj,rs=coupled_root(admitted.inputs,None)
                ...
                for key in native_chain:np.testing.assert_allclose(native_chain[key],oracle[key],rtol=0,atol=2e-12)
                chains.append(dict(role=role,inputs_object=self.obj(admitted.inputs.to_payload()),
                    selected_inputs_object=self.obj(admitted.selected.inputs.to_payload()),observed=native_chain,
                    independent_oracle={k:oracle[k].tolist() for k in native_chain},
                    hodge=[list(r) for r in admitted.selected.inputs.geometry.one_form_hodge.matrix],
                    source=[list(r) for r in admitted.selected.structural_source.increment]))
            self.assertNotEqual(chains[0]['observed']['current'],chains[1]['observed']['current'])
            self.assertNotEqual(chains[0]['observed']['j0'],chains[2]['observed']['j0'])
            ...
            self.assertLessEqual(selected.residual_squared,F(inputs.geometry.reference.profile.params_resolved.realization.tolerance)**2)
            stages.append(dict(label=label,inputs_object=self.obj(inputs.to_payload()),
                ...
                observed=observed,independent_oracle=expected,chains=chains,trial_points=trial_rows,
                certificate=bounds,residual_squared=str(selected.residual_squared),root_evaluations=root.evaluations,
                writer_count=writers.call_count,carrier_writes=carriers.call_count,
                continuity_evaluations=value.resource.continuity_evaluations,held_source_not_restart_source=True))
        limited=coupled_configure(self.inputs,limit=1,tolerance=0.)
        with self.assertRaises(ci.CIStageError) as exhausted:ci.CandidateCIRoot(limited)
        self.assertEqual(exhausted.exception.disposition,'no_admitted_root')
        obs=dict(stages=stages,source_policy='selected_root_source_held_once',
            candidate_history_or_W_writer=False,matched_forcing_contraction_claimed=False,
            indefinite_base_chart_invariance_claimed=False,global_root_uniqueness_claimed=False,
            composition_gain=2,rho_inst=1,gain_scope='constant_source_law_not_settled_trajectory',
            budget_control=dict(inputs_object=self.obj(limited.to_payload()),disposition=exhausted.exception.disposition,
        ...
        for name in ('CI-PC-SAME-SOURCE','C-POST-CONTINUITY-REDERIVATION'):
            ...
            if error:
                ...
                def injected(inputs, backend):
                    if inputs.current == self.inputs.current:
                        raise CandidateCStageError(error, 'test-only consumed CI solve fault')
                ...
                if dt > 0:
                    current = float(coupled_root(self.inputs, self.backend)[1][0])
    ...
    def test_fixed_profile_lifecycle_and_parent_ownership(self):
        ...
        with TemporaryDirectory(prefix='c-cipc-state-') as directory:
    ...
    def test_candidate_fixed_stage_and_exact_controls(self):
        ...
        for field,value,name in (('chi_C',0.,'C-CHI-ZERO'),('zeta_C',0.,'C-ZETA-ZERO'),('chi_C',.5,'C-ONE-CHI-GATE')):
            ...
            root=None
            if value==0:
                root=ci.ProvisionalCandidateCIStep(control)
                self.assertEqual(root.root.current,root.root.selected.point.algebra.baseline)
                self.assertEqual(root.root.selected.structural_source.increment,((0.,),))
            ...
            row(name,control_inputs_object=self.obj(control.to_payload()),control_complete_profile_id=ref.profile.complete_profile_id,
                ...
                zero_selected_inputs_object=None if root is None else self.obj(root.root.selected.inputs.to_payload()),
                zero_current=None if root is None else list(root.root.current.values),
                zero_source=None if root is None else [list(r) for r in root.root.selected.structural_source.increment],
