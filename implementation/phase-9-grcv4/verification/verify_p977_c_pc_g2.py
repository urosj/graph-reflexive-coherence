"""Bounded integrated C_PC G2 proposal; no acceptance or registry mutation.
...
import verify_p977_c_pc_acceptance as acceptance

SCRIPT = p.HERE + 'verify_p977_c_pc_g2.py'
TEST = p.HERE + 'test_p977_c_pc_g2.py'
INTERFACE = p.PHASE + 'tranche-7/P9-7.7-C_PC-G2Interface.json'
RECORD = p.PHASE + 'tranche-7/P9-7.7-C_PC-G2Review.json'
NOMINATED = crossing.NOMINATED
INTERFACE_TEST = 'test_p977_c_pc_g2.InterfaceTests.test_exact_profile_configuration_strict_run_and_missing_defaults'
...
def validate_interface(value):
    p.require(value['schema'] == 'phase9_c_pc_g2_interface_execution_v1'
...
def capture_interface():
    from test_p977_c_pc_g2 import InterfaceTests
    ...
    value = dict(schema='phase9_c_pc_g2_interface_execution_v1', captured_at=datetime.now(timezone.utc).isoformat(),
...
def build(bounded_acceptance=None):
    from c_pc_g2_source_reuse import retained_bindings
    ...
    product = []
    for case in sorted(review.required_cases('C_PC')):
    ...
    sources.update({n: p.sha((p.ROOT/n).read_bytes()) for n in (INTERFACE, crossing.RECORD,
        acceptance.REVIEW, p.HERE+'verify_p977_c_pc_acceptance.py', 'specs/grc-common-interface.md',
        'specs/grc-v4-a-initializer-spec.md', 'specs/grc-v4-topology-event-spec.md')})
    value = dict(schema='phase9_c_pc_integrated_g2_review_v1', gate='P9-G2[C_PC]', verdict='PASS_PROPOSAL',
...
def validate(value, expected):
    ...
    p.require([r['fixture_id'] for r in value['catalog_product']] == sorted(review.required_cases('C_PC')),
