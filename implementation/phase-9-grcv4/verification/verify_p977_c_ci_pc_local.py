"""Retained exact C_CI_PC local product; --run emits a new run without writing."""
...
SCRIPT=p.HERE+'verify_p977_c_ci_pc_local.py'
TEST=p.HERE+'test_p977_c_ci_pc_local.py'
RECORD=p.PHASE+'tranche-7/P9-7.7-C_CI_PC-LocalProduct.json'
...
def required():
    ...
    return sorted([r['id'] for r in catalog['common_cases']]+[r['id'] for r in catalog['candidate_c_cases']
                  if r['id']!='C-LIFECYCLE-REFERENCE-MAP']+['CI-PC-SAME-SOURCE']+sorted(LOCAL_LIFECYCLE))
...
def bindings():
    names={SCRIPT,TEST,review.RECORD,review.SCRIPT,review.CATALOG,
           ...
           p.PHASE+'tranche-6/P9-6.3ab-ExecutionRecord.json',p.PHASE+'tranche-6/P9-6.3abc-AuditFollowup.json',
           'tests/models/test_grc_v4_cipc.py','tests/models/test_grc_v4_ci.py','tests/models/test_grc_v4_pc.py','tests/models/test_grc_v4_candidate_c.py'}
    return {**runtime_bindings(),**{n:p.sha((p.ROOT/n).read_bytes()) for n in sorted(names)}}


def roster():
    from test_p977_c_ci_pc_local import CCIPCLocalProductTests
    return ['test_p977_c_ci_pc_local.CCIPCLocalProductTests.'+n
            for n in unittest.TestLoader().getTestCaseNames(CCIPCLocalProductTests)]
...
def authority():
    ...
    return {key: contract_provenance(context, key) for key in (
        'D10.2-EC-PARENT-REAL-CI-PC', 'D10.2-EC-CI-PC-C-COMPOSITION',
        'D10.2-EC-CI-C-CONTRACTION', 'D10.2-EC-CI-PC-ABLATIONS', 'D10.2-EC-PC-ZOH-WRITER',
...
def validate(value):
    ...
    from test_p977_c_ci_pc_local import NOMINATED
    p.require(value['schema']=='phase9_exact_profile_local_product_v1'
              and value['iteration_id']=='P9-7.7-C_CI_PC-local'
              and value['record_digest']==p.digest_record(value),'C_CI_PC local record drift')
    p.require(p.g2_bindings_match(value['source_bindings'], bindings()),'C_CI_PC local execution source drift')
    p.require(value['authority'] == authority(), 'C_CI_PC authority drift or support promotion')
    ...
    seed=review.read(review.SOURCES['lifecycle'])['families']['C_CI_PC']
    nomination=seed['initial']['reference']['profile']
    p.require(value['nomination']==nomination and nomination['complete_profile_id']==NOMINATED,'C_CI_PC nomination replaced')
    p.require(value['test_ids']==roster() and value['results']==dict(tests_run=5,failures=[],errors=[],skips=[]),
              'C_CI_PC local execution incomplete')
    ...
    for key,obj in objects.items():
        p.require(p.sha(canonical_json_bytes(obj))==key,'C_CI_PC evidence object changed')
...
def validate_science(value, seed):
    ...
    for name,field,target in (('C-CHI-ZERO','chi_C',0.),('C-ZETA-ZERO','zeta_C',0.),('C-ONE-CHI-GATE','chi_C',.5)):
        ...
        if target==0:
            ...
            selected=inputs(control['zero_selected_inputs_object'])
            p.require(selected.current==declared.current and selected.reset==declared.reset
                      and selected.geometry.reference==declared.geometry.reference and selected.stage=='cipc_trial',
                      'zero gate selected foreign authority')
            from tests.models.test_grc_v4_cipc import independent_root
            h,j,source=independent_root(declared,None)
            close(selected.geometry.one_form_hodge.matrix,h,'zero gate retains old-Z geometry')
            close(control['zero_current'],j,'zero gate coupled current')
            close(control['zero_source'],source,'zero gate source')
            p.require(np.count_nonzero(source)==0 and not np.array_equal(h,np.asarray(declared.geometry.reference.pairings.one_form.matrix)),
                      'zero gate erased persistent geometry')
        else:
...
def validate_pc(value,seed):
    """Complete C algebra at every retained trial/role plus literal coupled laws."""
    from dataclasses import replace
    from fractions import Fraction as F
    import numpy as np
    from pygrc.models.grc_v4_geometry import GeometryStageInputs
    from test_p977_c_ci_pc_local import independent_beat,old_h
    from tests.models.test_grc_v4_cipc import independent_root
    from tests.models.test_grc_v4_candidate_c import dense_current_oracle
    from tests.models.test_grc_v4_pc import zoh_oracle
    objects=value['objects'];nomination=value['nomination']
    rows={r['fixture_id']:r for r in value['fixture_results']}
    branch=rows['CI-PC-SAME-SOURCE']['observation']
    def inputs(key):return GeometryStageInputs.from_payload(objects[key])
    def close(a,b,name,atol=2e-11):
        p.require(np.shape(a)==np.shape(b) and np.allclose(a,b,rtol=0,atol=atol),'C_CI_PC equation drift: '+name)
    p.require(branch==rows['C-POST-CONTINUITY-REDERIVATION']['observation']
              ...
              and branch['global_root_uniqueness_claimed'] is False
              ...
              and branch['gain_scope']=='constant_source_law_not_settled_trajectory'
              and branch['scope']=='exact_profile_seed_and_same_profile_signed_history; separate_dense_fixed_stage_controls',
              'coupled source/history/claim ceiling changed')
    p.require([r['label'] for r in branch['stages']]==['nomination_seed','signed_history_companion'],'missing coupled state discrimination')
    expected_keys={'projector','sector','hm','phi','j0','ident','q','current','read'}
    for row in branch['stages']:
        ...
        p.require(recipe['inputs']==subject.to_payload() and recipe['differential_reference'] is None
                  ...
                  and following.geometry.reference==subject.geometry.reference,'coupled nomination/backend drift')
        ...
        p.require(subject.to_payload()['current']==current and subject.to_payload()['reset']==reset,'companion changed undeclared authority')
        expected=independent_beat(subject)
        p.require(set(row['observed'])==set(row['independent_oracle'])==set(expected),'missing coupled equations')
        for key in expected:
            close(row['observed'][key],expected[key],key,2e-11 if key in ('h','current','source') else 2e-13)
            close(row['independent_oracle'][key],expected[key],'oracle '+key,2e-13)
        close(row['observed']['h'],old_h(subject)+subject.geometry.reference.profile.params_resolved.geometry.kappa_H*np.asarray(row['observed']['source']),'literal old-Z plus immediate source')
        p.require(np.linalg.norm(np.asarray(row['observed']['h'])-old_h(subject))>1e-8,'PC-only geometry substituted')
        # Commit quantities consume the selected numerical root, not a new solve.
        incidence=np.asarray(subject.geometry.reference.graph.incidence)
        consumed=row['observed']['current']
        final=[float(F(v)-F(subject.dt)*sum(F(float(incidence[i,k]))*F(float(consumed[k])) for k in range(len(consumed)))) for i,v in enumerate(subject.current.C)]
        written=zoh_oracle(subject.current.Z_4,tuple(np.asarray(row['observed']['source']).flat),subject.dt,subject.geometry.reference.profile.params_resolved.realization.tau_PC)
        close(following.current.C,final,'written C',2e-13)
        close(following.current.Z_4,written,'written Z',2e-14)
        p.require(following.current.W_A is None and following.reset==subject.reset
                  ...
                  and row['held_source_not_restart_source'] is True,'coupled writer/readmission staging changed')
        p.require(not np.allclose(row['observed']['source'],independent_root(following,None)[2],rtol=0,atol=1e-10),'post-continuity source not distinguished')
        trials=row['trial_points']
        p.require(len(trials)>1,'missing current trial rebuilds')
        for trial in trials:
            admitted=inputs(trial['inputs_object'])
            p.require(admitted.current==subject.current and admitted.reset==subject.reset
                      and admitted.geometry.reference==subject.geometry.reference and admitted.stage=='cipc_trial',
                      'trial borrowed new C/Z or foreign reference')
            oracle=dense_current_oracle(admitted)
            p.require(set(trial['observed'])==set(trial['independent_oracle'])==expected_keys,'incomplete C trial chain')
            for key in expected_keys:
                close(trial['observed'][key],oracle[key],'trial '+key,2e-12)
                close(trial['independent_oracle'][key],oracle[key],'trial oracle '+key,2e-12)
        p.require([r['role'] for r in row['chains']]==['consumed','reset','restart'],'missing coupled role chain')
        for chain in row['chains']:
            admitted=inputs(chain['inputs_object']);selected=inputs(chain['selected_inputs_object'])
            expected_state={'consumed':subject.current,'reset':subject.reset,'restart':following.current}[chain['role']]
            p.require(admitted.current==selected.current==expected_state
                      and admitted.geometry.reference==selected.geometry.reference==subject.geometry.reference
                      and admitted.reset==selected.reset==subject.reset
                      and admitted.stage=='pre_read' and selected.stage=='cipc_trial','borrowed C readmission inputs')
            h,j,s=independent_root(admitted,None)
            close(chain['hodge'],h,'role joint hodge');close(chain['source'],s,'role joint source')
            close(selected.geometry.one_form_hodge.matrix,h,'selected role geometry')
            close(chain['observed']['current'],j,'selected role current')
            oracle=dense_current_oracle(selected)
            p.require(set(chain['observed'])==set(chain['independent_oracle'])==expected_keys,'incomplete C role chain')
            for key in expected_keys:
                close(chain['observed'][key],oracle[key],chain['role']+' '+key,2e-12)
                close(chain['independent_oracle'][key],oracle[key],'oracle '+chain['role']+' '+key,2e-12)
        p.require(row['chains'][0]['observed']['j0']!=row['chains'][2]['observed']['j0']
                  and row['chains'][0]['observed']['current']!=row['chains'][1]['observed']['current']
                  and trials[0]['observed']['j0']!=row['chains'][0]['observed']['j0'],'C reconstruction stages collapsed')
        cert=row['certificate'];params=nomination['params_resolved']['realization']
        p.require(cert['scope']=='analytic_local_reference_ball'
                  and F(cert['contraction_upper'])<1
                  and F(cert['displacement_upper'])<=F(cert['radius'])
                  and F(cert['uniform_source_upper'])<=F(cert['carrier_radius'])==F(params['radius'])
                  and F(cert['uniform_source_slack'])==F(cert['carrier_radius'])-F(cert['uniform_source_upper'])
                  and cert['rho_inst']==1 and F(row['residual_squared'])<=F(params['tolerance'])**2
                  and 1<row['root_evaluations']<=params['iteration_limit'],'coupled root/envelope not admitted')
        p.require(float(np.linalg.norm(row['observed']['source']))<=float(F(cert['uniform_source_upper'])),'source outside envelope')
    budget=branch['budget_control'];limited=inputs(budget['inputs_object'])
    p.require(budget['disposition']=='no_admitted_root' and budget['fallback_used'] is False
              ...
              and limited.geometry.reference.profile.complete_profile_id!=nomination['complete_profile_id']
              and limited.geometry.reference.profile.params_resolved.realization.iteration_limit==1
              and limited.geometry.reference.profile.params_resolved.realization.tolerance==0,
              'budget control promoted or fallback hidden')
    ...
    for row in pressure:
        ...
        p.require(before==after and before['reference']['profile']==nomination
                  ...
                  'C_CI_PC rejection/rollback changed')
...
def check(initial_review=None):
    ...
    return dict(status='local_product_verified_pending_review',record_path=RECORD,record_digest=value['record_digest'],
        ...
        remaining_catalog_cases=sorted(set(review.required_cases('C_CI_PC'))-set(required())),
...
def run():
    ...
    p.require(result.wasSuccessful() and not result.skipped,'C_CI_PC local product tests failed')
    p.require(bindings()==sources,'sources changed during C_CI_PC capture')
    seed=review.read(review.SOURCES['lifecycle'])['families']['C_CI_PC']
    value=dict(schema='phase9_exact_profile_local_product_v1',iteration_id='P9-7.7-C_CI_PC-local',
        ...
        fixture_results=CCIPCLocalProductTests.rows,objects=CCIPCLocalProductTests.objects,
        readmission_pressure=CCIPCLocalProductTests.pressure,
        ...
        scope='26 local C_CI_PC catalog cells; seven crossing cells remain. Control profiles and provisional stages are not additional supported profiles or committed operations; shared executions are not added together.')
