"""Retained C_CI_PC evidence pressure; no numerical campaign reruns."""

from copy import deepcopy
import unittest
from unittest.mock import patch
import verify_p977_c_ci_pc_local as audit


class CCIPCLocalEvidenceTests(unittest.TestCase):
    def test_retained_product_and_rehashed_mutations(self):
        ...
        def pc(value,change):
            ...
            for case in ('CI-PC-SAME-SOURCE','C-POST-CONTINUITY-REDERIVATION'):change(obs(value,case))
        changes={
            ...
            'borrowed_profile':lambda v:v['fixture_results'][0].update(complete_profile_id='C_CI_PC'),
            ...
            'source_envelope':lambda v:pc(v,lambda b:b['stages'][0]['certificate'].update(uniform_source_upper='9999999')),
            'PC_only_policy':lambda v:pc(v,lambda b:b.update(source_policy='held_old_prestate_source_not_post_continuity_refresh')),
            'double_gain':lambda v:pc(v,lambda b:b.update(composition_gain=1)),
            'global_root_overclaim':lambda v:pc(v,lambda b:b.update(global_root_uniqueness_claimed=True)),
            'settled_trajectory_overclaim':lambda v:pc(v,lambda b:b.update(gain_scope='observed_settled_trajectory')),
            'unbounded_root':lambda v:pc(v,lambda b:b['stages'][0]['certificate'].update(contraction_upper='1')),
            'missing_trials':lambda v:pc(v,lambda b:b['stages'][0]['trial_points'].clear()),
            'stale_trial_baseline':lambda v:pc(v,lambda b:b['stages'][0]['trial_points'][0]['observed'].update(j0=[999.])),
            'budget_fallback':lambda v:pc(v,lambda b:b['budget_control'].update(fallback_used=True)),
            'zero_source':lambda v:obs(v,'C-CHI-ZERO').update(zero_source=[[1.]]),
            'zero_reset_geometry':lambda v:obs(v,'C-ZETA-ZERO').update(zero_selected_inputs_object=obs(v,'C-ZETA-ZERO')['control_inputs_object']),
