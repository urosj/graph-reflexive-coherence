"""C_RG2b crossing evidence and generic projection; no native campaign rerun."""

from copy import deepcopy
import unittest
from unittest.mock import patch
from pygrc.models.grc_v4_codec import canonical_json_bytes
import verify_p977_c_rg2b_crossings as audit


class CRG2bCrossingEvidenceTests(unittest.TestCase):
    def test_rehashed_endpoint_history_completion_and_scope_pressure(self):
        ...
        def object_edit(v,key,edit):
            ...
            new=audit.p.sha(canonical_json_bytes(obj));v['objects'][new]=obj
            def rewrite(x):
        ...
        mutations={
            ...
            'borrowed_source':lambda v:row(v,'mapped_event').update(complete_profile_id='C_RG2b'),
            ...
            'loss_erased':lambda v:obs(v,'candidate_incoming').update(expected_losses=[]),
            ...
            'initializer_reference':lambda v:obs(v,'initializer_outgoing')['expectations']['reset'].update(reference_pass={}),
            'missing_target_reference':lambda v:obs(v)['complete_target_reference'].pop('target-e001'),
            'incomplete_C_chain':lambda v:read(v)['C_chain'].pop('q'),
            'borrowed_baseline':lambda v:read(v,'reset')['C_chain'].update(j0=read(v)['C_chain']['j0']),
            'false_initializer_loss':lambda v:obs(v,'initializer_outgoing').update(expected_losses=['candidate_history_loss']),
            ...
            'invented_event_initializer':lambda v:obs(v).update(no_initializer=False),
            ...
            'not_reset_only':lambda v:obs(v,'readmission_rejection').update(reset_C=[2.]*4),
        ...
        with patch.object(audit,'authority',return_value=value['authority']):
            for name,edit in mutations.items():
                ...
                # Inner preimages use the runtime codec, not the audit-record
                # digest format: rejection must survive valid object hashes.
                self.assertTrue(all(k==audit.p.sha(canonical_json_bytes(o)) for k,o in bad['objects'].items()))
    ...
    def test_registered_crossing_checker_and_browser_projection(self):
        ...
        checked=registry._checker(root,'verify_p977_c_rg2b_acceptance')(
            local_product={'record_digest':record['local_record_digest']})
        roster=registry.registry(root)
        self.assertIn(dict(view_key='c_rg2b_crossings',module='verify_p977_c_rg2b_acceptance',kind='crossing',
                           dependency='c_rg2b_local_product'),roster['materializers'])
        views=dict(roster['reconciliation_views'],c_rg2b_crossings=checked)
...
const r=body.rows.find(r=>r.children[0].textContent==='c_rg2b_crossings');assert.ok(r);
...
for(const field of ['G2_accepted','G3_accepted','user_accepted','aggregate_closed','all_ordered_pairs_verified']) {
 const bad=structuredClone(v);bad.c_rg2b_crossings[field]=!bad.c_rg2b_crossings[field];assert.throws(()=>checkedReconciliation(bad,true));
}
for(const field of ['reconciled_crossing_cells','new_execution_cases','retained_alias_count','numerical_tests_rerun']) {
 const bad=structuredClone(v);bad.c_rg2b_crossings[field]++;assert.throws(()=>checkedReconciliation(bad,true));
}
const bad=structuredClone(v);delete bad.c_rg2b_crossings;assert.throws(()=>checkedReconciliation(bad,true));
checkedReconciliation({},false);assert.throws(()=>checkedReconciliation(v,false));
console.log('C_RG2B_CROSSING_PROJECTION_PASS');
"""
        ...
        self.assertEqual(result.returncode,0,result.stderr);self.assertIn('C_RG2B_CROSSING_PROJECTION_PASS',result.stdout)
