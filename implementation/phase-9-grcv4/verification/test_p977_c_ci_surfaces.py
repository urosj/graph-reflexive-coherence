...
class CCISurfaceTests(unittest.TestCase):
    def test_actual_status_and_cached_transport_agreement(self):
        ...
        self.assertEqual(proposal['status'],'pass_proposal_pending_G2_acceptance')
        self.assertEqual(proposal['catalog_cells'],28)
        self.assertFalse(proposal['G2_accepted'])
        self.assertFalse(proposal['user_accepted'])
        self.assertEqual(proposal['new_G2_support'],[])
        self.assertEqual(proposal['accepted_support_unchanged'],sorted(status['accepted_generic_runtime_support']))
...
assert.deepEqual(body.children.map(r=>r.children[2].textContent),['G2 accepted','G2 accepted','G2 accepted','G2 accepted','G2 proposal — not accepted']);
...
for(const edit of [v=>delete v.a_pc_g2_review,v=>v.a_pc_g2_review.G2_accepted=true,
 v=>v.a_pc_g2_review.user_accepted=true,v=>v.a_pc_g2_review.catalog_cells=27,
 ...
 v=>v.profile_g2[4].state='accepted',v=>v.accepted_generic_runtime_support.push(v.a_pc_g2_review.proposed_additional_support[0])]) {
 const bad=structuredClone(value);edit(bad);delete bad.status_digest;
 bad.status_digest=createHash('sha256').update(canonical(bad)).digest('hex');
 await assert.rejects(verifiedStatus(bad));
}
console.log('A_PC_G2_PROPOSAL_BROWSER_PASS');
"""
        ...
        self.assertIn('A_PC_G2_PROPOSAL_BROWSER_PASS',browser.stdout)
