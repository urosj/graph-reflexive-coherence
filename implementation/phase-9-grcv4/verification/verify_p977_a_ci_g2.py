"""Bounded integrated A_CI G2 proposal; no acceptance or registry mutation.
...
import verify_p977_a_ci_acceptance as acceptance

SCRIPT = p.HERE + 'verify_p977_a_ci_g2.py'
TEST = p.HERE + 'test_p977_a_ci_g2.py'
INTERFACE = p.PHASE + 'tranche-7/P9-7.7-A_CI-G2Interface.json'
RECORD = p.PHASE + 'tranche-7/P9-7.7-A_CI-G2Review.json'
NOMINATED = crossing.NOMINATED
INTERFACE_TEST = 'test_p977_a_ci_g2.InterfaceTests.test_exact_profile_configuration_strict_run_and_missing_defaults'
...
def capture_interface():
    from test_p977_a_ci_g2 import InterfaceTests
    ...
    value = dict(schema='phase9_a_ci_g2_interface_execution_v1', captured_at=datetime.now(timezone.utc).isoformat(),
...
def authority():
    ...
    contracts = ('D10.2-EC-CI-A-ROOT', 'D10.2-EC-CI-A-CONTRACTION', 'P9-EC-A-INITIALIZER-REFERENCE-PASS', 'P9-EC-RECEIPT-PARENT-CHAIN',
...
def build(bounded_acceptance=None):
    ...
    product = []
    for case in sorted(review.required_cases('A_CI')):
    ...
    initializer_target = next(r['target_active_model_identity'] for r in b['retained_aliases'] if r['key'] == 'initializer/A_CI')
    sources = interface_bindings()
    sources.update({n: p.sha((p.ROOT/n).read_bytes()) for n in (INTERFACE, crossing.RECORD,
        acceptance.REVIEW, p.HERE+'verify_p977_a_ci_acceptance.py', 'specs/grc-common-interface.md',
    ...
    value = dict(schema='phase9_a_ci_integrated_g2_review_v1', gate='P9-G2[A_CI]', verdict='PASS_PROPOSAL',
        ...
        ordered_scope=dict(matrix=b['ordered_migration_matrix'], C_to_nominated_A='rejected_unselected_initializer',
            ...
            separate_PC_pairs_not_A_CI_credit=True,
...
def validate(value, expected):
    ...
    p.require([r['fixture_id'] for r in value['catalog_product']] == sorted(review.required_cases('A_CI')),
