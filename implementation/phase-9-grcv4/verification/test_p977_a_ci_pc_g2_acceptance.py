"""Exact A_CI_PC decision, unchanged evidence, and finite source-reuse pressure."""
...
import a_ci_pc_g2_source_reuse as reuse
...
class AcceptanceTests(unittest.TestCase):
    def test_exact_discovery_and_no_control_or_target_promotion(self):
        ...
        row = next(r for r in g.registry(p.ROOT)['records'] if r['profile_family_id']=='A_CI_PC')
        self.assertEqual(row['profile_family_id'], 'A_CI_PC')
    ...
    def test_review_runs_and_prior_acceptances_unchanged(self):
        ...
        names = [prefix+'A_CI_PC-'+suffix for suffix in (
            'G2Review.json', 'G2Review.md', 'G2Interface.json', 'LocalProduct.json', 'Crossings.json')]
        names += [prefix+family+'-'+suffix for family in ('A_CI', 'A_OS', 'C_CI', 'A_PC', 'C_PC')
    ...
    def test_current_integrated_review_projects_exact_acceptance(self):
        ...
        self.assertEqual(projected['catalog_cells'], 28)
