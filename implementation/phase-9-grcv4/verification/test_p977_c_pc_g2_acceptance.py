"""Exact C_PC decision, unchanged evidence, and finite source-reuse pressure."""
...
import c_pc_g2_source_reuse as reuse
...
class AcceptanceTests(unittest.TestCase):
    def test_exact_discovery_and_no_control_or_target_promotion(self):
        ...
        self.assertEqual(row['profile_family_id'], 'C_PC')
    ...
    def test_review_runs_and_prior_acceptances_unchanged(self):
        ...
        names = [prefix+'C_PC-'+suffix for suffix in (
            'G2Review.json', 'G2Review.md', 'G2Interface.json', 'LocalProduct.json', 'Crossings.json')]
        names += [prefix+family+'-'+suffix for family in ('A_CI', 'A_OS', 'C_CI', 'A_PC')
