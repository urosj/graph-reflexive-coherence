"""Exact A_PC crossing reconciliation. --run emits a fresh record to stdout."""
...
import verify_p977_a_pc_local as local

SCRIPT = p.HERE + 'verify_p977_a_pc_crossings.py'
TEST = p.HERE + 'test_p977_a_pc_crossings.py'
RECORD = p.PHASE + 'tranche-7/P9-7.7-A_PC-Crossings.json'
...
def roster():
    from test_p977_a_pc_crossings import APCCrossingTests
    return ['test_p977_a_pc_crossings.APCCrossingTests.' + n
            for n in unittest.TestLoader().getTestCaseNames(APCCrossingTests)]
...
def retained():
    ...
    aliases = []
    for origin, key in [('migration', k) for k in ('A_NH_NH', 'A_NH_PC', 'A_PC_NH', 'A_PC_CIPC', 'A_CIPC_PC', 'A_C_PC')] + [('initializer', 'A_PC')]:
...
def matrix():
    return [
        dict(migration_class='same_candidate_nonhistory_to_nonhistory', disposition='separate_endpoints_not_A_PC; no positive nomination credit', evidence='migration/A_NH_NH'),
        dict(migration_class='same_candidate_nonhistory_to_history', disposition='positive_exact_nomination_target', evidence='migration/A_NH_PC'),
        dict(migration_class='same_candidate_history_to_nonhistory', disposition='positive_exact_nomination_source', evidence='migration/A_PC_NH'),
        dict(migration_class='PC_to_CI_PC', disposition='positive_exact_nomination_source; exact_carrier_contract_only', evidence='migration/A_PC_CIPC', negative='carrier_contract_rejection'),
        dict(migration_class='CI_PC_to_PC', disposition='positive_exact_nomination_target; exact_carrier_contract_only', evidence='migration/A_CIPC_PC'),
        dict(migration_class='A_to_C', disposition='positive_exact_nomination_source; separate_candidate_loss_and_whole_carrier_reset', evidence='migration/A_C_PC'),
        dict(migration_class='C_to_A', disposition='negative_exact_nomination_target; positive initializer target remains a different profile',
             evidence='incoming_C_rejection', separate_positive='initializer/A_PC'),
    ]


def cells():
    return {
        'A-MIGRATION-HISTORY-RECEIPT': ['migration/A_NH_PC','migration/A_PC_NH','migration/A_PC_CIPC','migration/A_CIPC_PC','migration/A_C_PC'],
        'RESET-AFTER-MIGRATION': ['migration/A_NH_PC','migration/A_PC_NH','migration/A_PC_CIPC','migration/A_CIPC_PC','migration/A_C_PC'],
        'RESET-AFTER-EVENT': ['mapped_event'],
        'WHOLE-LIFECYCLE-TUPLE-MAP': ['mapped_event'],
        'ALL-MIGRATION-CLASSES': ['ordered_migration_matrix'],
        'HISTORY-DISPOSITION': ['migration/A_NH_PC','migration/A_PC_NH','migration/A_PC_CIPC','migration/A_CIPC_PC','migration/A_C_PC','mapped_event','carrier_contract_rejection'],
...
def validate_retained_maps():
    ...
    for key in ('A_NH_PC','A_PC_NH','A_PC_CIPC','A_CIPC_PC','A_C_PC'):
        row = cases[key]
        before, after = row['before'], row['after']
        source = Reference.from_payload(row['initial']['reference'])
        target = Reference.from_payload(row['target_reference'])
        endpoint = target if key in ('A_NH_PC','A_CIPC_PC') else source
        ...
        expected_losses = (['candidate_history_loss','carrier_history_loss'] if key=='A_C_PC' else
                           ['carrier_history_loss'] if key=='A_PC_NH' else [])
        expected_carrier = {'A_NH_PC':'target_initializer','A_PC_NH':'explicit_loss',
            'A_PC_CIPC':'exact_transport','A_CIPC_PC':'exact_transport','A_C_PC':'whole_carrier_reset'}[key]
        p.require(losses==expected_losses and history['carrier']['disposition']==expected_carrier
                  and history['candidate']['disposition']==('explicit_loss' if key=='A_C_PC' else 'exact_transport'),
                  'retained two-channel history policy mismatch')
        if key in ('A_PC_CIPC','A_CIPC_PC'):
            p.require(canonical_json_bytes(carrier_contract(source))==canonical_json_bytes(carrier_contract(target)),
                      'PC pair did not preserve exact carrier contract')
        for role in ('scientific_state','reset'):
            a,b=before[role]['authoritative'],after[role]['authoritative']
            z = None if key=='A_PC_NH' else [0.] if key in ('A_NH_PC','A_C_PC') else a['Z_4']
            p.require(b==dict(C=a['C'],W_A=None if key=='A_C_PC' else a['W_A'],Z_4=z), 'retained role map mismatch')
        if key!='A_NH_PC':
            a,b=before['scientific_state']['authoritative'],before['reset']['authoritative']
            p.require(a['Z_4']!=b['Z_4'] and a['Z_4']!=[0.] and b['Z_4']!=[0.], 'vacuous retained carrier control')
        for name in ('time','step_index','Q_target'):
            p.require(before['scientific_state'][name]==after['scientific_state'][name], 'retained migration clock/charge drift')
...
def validate_case(case,row,before,after,request,result,obs,objects):
    ...
    if case=='incoming_C_rejection':
        p.require(request['target_profile_id']==obs['failed_target_profile_id']==NOMINATED
                  ...
                  and obs['selected_initializer_profile_id']==initializer_target('A_PC')[0].profile.complete_profile_id
    ...
    if case=='mapped_event':
        ...
        selected,selected_backend=initializer_target('A_PC')
        ...
        for role,key in (('current','scientific_state'),('reset','reset')):
            ...
            current,_=independent_point(inputs,backend,h)
            close(expect['h'],h,'event zero-carrier geometry mismatch')
            close(inputs.geometry.one_form_hodge.matrix,h,'event readmission geometry mismatch')
...
def validate(value):
    ...
    p.require(value['schema'] == 'phase9_exact_profile_crossing_reconciliation_v1'
              and value['iteration_id'] == 'P9-7.7-A_PC-crossings'
    ...
    p.require(set(cells()) == set(review.required_cases('A_PC')) - set(local.required()), 'crossing catalog drift')
...
def check(local_product=None):
    ...
    return dict(status='crossings_reconciled_pending_review', record_path=RECORD, record_digest=value['record_digest'],
        complete_profile_id=NOMINATED, test_count=3, new_execution_cases=5, retained_alias_count=7,
...
def run():
    ...
    value = dict(schema='phase9_exact_profile_crossing_reconciliation_v1', iteration_id='P9-7.7-A_PC-crossings',
        ...
        fixture_results=APCCrossingTests.rows, objects=APCCrossingTests.objects,
