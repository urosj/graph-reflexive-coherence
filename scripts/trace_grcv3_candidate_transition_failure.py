"""Trace why one rich.v4 lane reaches spark candidates while another does not."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Trace the candidate-transition boundary between two rich.v4 "
            "transfer-mediation lanes."
        )
    )
    parser.add_argument(
        "--baseline-seed",
        default=str(DEFAULT_GRCV3_CANDIDATE_TRANSITION_BASELINE_SEED),
        help="Baseline lane seed path.",
    )
    parser.add_argument(
        "--comparison-seed",
        default=str(DEFAULT_GRCV3_CANDIDATE_TRANSITION_COMPARISON_SEED),
    ...
    parser.add_argument(
        "--steps",
        type=int,
        default=DEFAULT_GRCV3_CANDIDATE_TRANSITION_STEPS,
...
def main(argv: list[str] | None = None) -> int:
    ...
    trace = build_grcv3_landscape_candidate_transition_trace(
