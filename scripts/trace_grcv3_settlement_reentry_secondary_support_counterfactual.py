"""Run decisive counterfactuals on descendant secondary support roles."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run descendant secondary-support counterfactuals for two rich.v4 "
            "settlement-regime lanes."
        )
    )
    parser.add_argument(
        "--baseline-seed",
...
def main(argv: list[str] | None = None) -> int:
    ...
    trace = build_grcv3_landscape_settlement_reentry_secondary_support_counterfactual_trace(
