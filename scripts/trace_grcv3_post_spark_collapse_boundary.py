...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Compare the saved rich-v4 post-spark collapse lane against the "
            "tightest direct transfer-mediation controls."
        )
    )
    parser.add_argument(
        "--baseline-seed",
    ...
    parser.add_argument(
        "--steps",
        type=int,
        default=DEFAULT_GRCV3_POST_SPARK_COLLAPSE_STEPS,
...
def main(argv: list[str] | None = None) -> int:
    ...
    trace = build_grcv3_landscape_post_spark_collapse_boundary_trace(
