"""Trace geometry-mediated rerouting away from the initial collapsed sink."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Trace how geometry shifts the blocked lane away from its initial "
            "collapsed sink without using any explicit anti-reentry rule."
        )
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    trace = build_grcv3_landscape_post_collapse_geometry_exclusion_trace(
