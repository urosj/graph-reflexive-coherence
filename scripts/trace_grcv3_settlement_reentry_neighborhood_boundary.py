"""Trace descendant neighborhood signatures at the settlement reentry boundary."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Trace descendant neighborhood signatures for two rich.v4 "
            "settlement-regime lanes at the post-split reentry boundary."
        )
    )
    parser.add_argument(
        "--baseline-seed",
    ...
    parser.add_argument(
        "--comparison-seed",
        default=str(DEFAULT_GRCV3_SETTLEMENT_REENTRY_COMPARISON_SEED),
...
def main(argv: list[str] | None = None) -> int:
    ...
    trace = build_grcv3_landscape_settlement_reentry_neighborhood_trace(
