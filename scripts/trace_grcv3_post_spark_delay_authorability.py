"""Explain whether the blocked late-cascade delay is already authored by existing transfer mediation."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Explain whether the blocked rich-v4 late-cascade delay is already "
            "authored by transfer_mediation.center_coupling_classes."
        )
    )
    parser.add_argument(
        "--baseline-seed",
...
def main(argv: list[str] | None = None) -> int:
    ...
    trace = build_grcv3_landscape_post_spark_delay_authorability_trace(
