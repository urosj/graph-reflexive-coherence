"""Render visuals for the representative Phase T-GRC9 replay lane."""
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Render artifact-driven visuals for the representative Phase T-GRC9 "
            "primary/replay lane. Graph-facing modes require saved port-graph "
            "checkpoints."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--experiment-path",
        default=str(DEFAULT_GRC9_REPRESENTATIVE_EXPERIMENT_PATH),
    ...
    parser.add_argument(
        "--lane-name",
        default=DEFAULT_GRC9_PHASE_T_REPRESENTATIVE_LANE,
        help="Representative GRC9 lane to render.",
    )
    parser.add_argument(
        "--surface",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        result = render_grc9_representative_visual_suite(
    ...
    print("Representative GRC9 visuals complete.")
