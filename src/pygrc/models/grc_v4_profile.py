...
def list_supported_profiles() -> frozenset[str]:
    ...
    return frozenset({_ACCEPTED_C_OS_ID, _ACCEPTED_A_OS_ID})
...
def get_supported_profile(complete_profile_id: str) -> GRCV4Profile:
    ...
    return GRCV4Profile.from_canonical_bytes(
        _ACCEPTED_C_OS_DECLARATION if complete_profile_id == _ACCEPTED_C_OS_ID else _ACCEPTED_A_OS_DECLARATION
    )
