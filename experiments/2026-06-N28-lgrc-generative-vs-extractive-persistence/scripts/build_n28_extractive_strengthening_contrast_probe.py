#!/usr/bin/env python3
"""Build N28 Iteration 4-C extractive strengthening contrast probe."""
...
OUTPUT = EXPERIMENT / "outputs" / "n28_extractive_strengthening_contrast_probe.json"
REPORT = EXPERIMENT / "reports" / "n28_extractive_strengthening_contrast_probe.md"
ARTIFACT_DIR = EXPERIMENT / "outputs" / "n28_extractive_strengthening_contrast_probe_artifacts"
SCRIPT_RELATIVE_PATH = (
    "experiments/2026-06-N28-lgrc-generative-vs-extractive-persistence/scripts/"
    "build_n28_extractive_strengthening_contrast_probe.py"
)
...
def build_traces(i3: dict[str, Any]) -> dict[str, Any]:
    ...
    threshold_policy = {
        "policy_id": "n28_i4c_extractive_strengthening_policy_v1",
    ...
    runtime_trace = {
        "run_artifact_id": "n28_i4c_extractive_strengthening_runtime_trace",
        "runtime_config_id": "n28_i4c_cross_shell_extractive_fixture_v1",
        ...
        "focal_basin_id": "n28_i4c_focal_basin_delta",
        "neighbor_scope_id": "n28_i4c_neighbor_capacity_shell_delta",
        "fixture_distinction_from_i4b": (
            "I4-C uses a cross-shell directional drain around the delta focal "
            "basin rather than the I4-B gamma local shell drain."
        ),
        "steps": [
            {
                "step": 0,
                ...
                "neighbor_distinguishability": 0.732,
                "neighbor_support_floor": 0.758,
                "neighbor_boundary_integrity": 0.719,
                "environment_basin_forming_capacity": 0.704,
            ...
            },
            {
                ...
                "focal_coherence_min": 0.878,
                "focal_stability_score": 0.878,
                "neighbor_distinguishability": 0.646,
                "neighbor_support_floor": 0.688,
                "neighbor_boundary_integrity": 0.629,
    ...
    focal_trace = {
        "trace_id": "n28_i4c_focal_basin_stability_trace",
        "focal_basin_id": runtime_trace["focal_basin_id"],
    ...
    neighbor_trace = {
        "trace_id": "n28_i4c_neighbor_capacity_trace",
        "neighbor_scope_id": runtime_trace["neighbor_scope_id"],
    ...
    extraction_trace = {
        "trace_id": "n28_i4c_extraction_leakage_trace",
    ...
    capacity_attribution_trace = {
        "trace_id": "n28_i4c_capacity_attribution_trace",
        ...
        "attribution_result": "source_current_cross_shell_neighbor_capacity_degradation_with_extractive_mechanism",
    }
    classification_trace = {
        "trace_id": "n28_i4c_extractive_strengthening_classification_trace",
...
def build_output() -> dict[str, Any]:
    ...
    row = {
        ...
        "row_decision_scope": "provisional_GE3_extractive_strengthening_measured_contrast_pending_replay_controls_and_neutral_contrasts",
        "ge_ladder_rung": "GE3",
        ...
        "shared_regime_policy_status_scope": (
            "generative_primary_and_strengthening_plus_primary_and_strengthening_extractive_contrasts_pending_neutral_contrasts"
        ),
        "policy_divergence_record": {
            "policy_id": traces["threshold_policy"]["policy_id"],
            "divergence_status": "same_policy_family_preserved_for_extractive_strengthening_contrast_pending_neutral_contrast_rows",
        ...
        "ap4_condition_reason": "I4-C extractive strengthening fixture does not make route-conditioned selection claim; AP4/N14 NAT4 gap remains inherited context.",
        "ap5_dependency_status": "not_applicable",
        "ap5_condition_reason": "I4-C extractive strengthening fixture does not use proxy/target formation as positive evidence; AP5/N15 NAT4 gap remains unresolved.",
        "claim_ceiling": "provisional_GE3_extractive_strengthening_measured_contrast_pending_replay_controls_neutral_contrasts_and_stress",
    ...
    checks = [
        ...
        {
            "check_id": "shared_policy_status_bounded_to_extractive_strengthening",
            "passed": row["shared_regime_policy_status"] == "partially_supported"
            and row["shared_regime_policy_status_scope"]
            == "generative_primary_and_strengthening_plus_primary_and_strengthening_extractive_contrasts_pending_neutral_contrasts",
        },
        {
            "check_id": "i4c_strengthens_i4b_without_replacement",
            "passed": row["extractive_strengthening_margin_comparison"][
                "all_load_bearing_extractive_margins_comparable_or_stronger"
            ]
            is True
            and row["extractive_strengthening_margin_comparison"][
                "thresholds_widened_relative_to_i4b"
            ]
            is False
            and row["extractive_strengthening_margin_comparison"]["i4b_replaced"]
            is False
            and row["extractive_strengthening_margin_comparison"][
                "i4b_imported_as_evidence"
            ]
            is False
            and row["extractive_contrast_record"]["strengthens_i4b"] is True,
        },
        {
    ...
    output = {
        "artifact_id": "n28_extractive_strengthening_contrast_probe",
...
def write_report(output: dict[str, Any]) -> None:
    ...
    comparison = row["extractive_strengthening_margin_comparison"]
    lines = [
        ...
        f"- I4-B strengthened, not replaced: `{str(output['i4b_strengthened_not_replaced']).lower()}`",
        f"- Ready for I4-D: `{str(output['ready_for_iteration_4d_primary_competitive_neutral_contrast']).lower()}`",
        "",
        "I4-C adds a second source-current extractive measured contrast whose "
        "job is to strengthen I4-B. It preserves focal viability while a "
        "distinct neighboring shell loses distinguishability, support, boundary "
        "integrity, and basin-forming capacity under exposed extraction, "
        "flattening, and merge/leakage. The row is corroborating extractive "
        "contrast evidence, not generative evidence.",
        "",
        "## Candidate Metrics",
        "",
        "```text",
        ...
        "## Interpretation",
        "",
        "Geometrically, I4-C repeats the I4-B extractive predicate in a distinct "
        ...
        "basin-forming capacity more strongly or comparably on every "
        "load-bearing extractive axis. The focal basin persists through a "
        "cross-shell drain: local support and coherence stay viable inside the "
        "focal basin while neighboring distinguishability, support, boundary "
        "integrity, and environment capacity are drawn down.",
        "",
        "This strengthens I4-B because it does not merely change labels or rerun "
        "the same trace. I4-B used the gamma shell. I4-C uses a separate delta "
        "cross-shell geometry and still lands in the same extractive regime "
        "under the same frozen policy family. The result is stronger evidence "
        "that extractive persistence is a measurable regime boundary, not a "
        "single fixture artifact.",
        "",
        "Compactly: I4-B establishes focal persistence with neighbor capacity "
        "loss once; I4-C shows the same extractive relation again with a "
        "distinct topology/shell setup and comparable or stronger margins.",
        "",
        "I4-C consumes I4-B only for margin comparison. It does not import I4-B "
        "as evidence, replace it, widen thresholds, or promote extractive "
        ...
        f"strengthens_i4b = {str(row['extractive_contrast_record']['strengthens_i4b']).lower()}",
        ...
        "## I4-B Comparison",
        "",
        f"- All load-bearing extractive margins comparable or stronger: `{str(comparison['all_load_bearing_extractive_margins_comparable_or_stronger']).lower()}`",
        f"- Thresholds widened relative to I4-B: `{str(comparison['thresholds_widened_relative_to_i4b']).lower()}`",
        f"- I4-B imported as evidence: `{str(comparison['i4b_imported_as_evidence']).lower()}`",
        f"- I4-B replaced: `{str(comparison['i4b_replaced']).lower()}`",
        "",
        "| Axis | I4-B | I4-C | Relation |",
        "|---|---:|---:|---|",
    ]
    for item in comparison["axes"]:
        lines.append(
            f"| `{item['axis']}` | `{item['i4b_value']}` | `{item['i4c_value']}` | `{item['margin_relation']}` |"
        )
    ...
    lines.extend(
        [
            ...
            "## Claim Boundary",
            "",
            "I4-C does not support GE4+, GE5+, GE6, final N28, semantic cooperation, "
