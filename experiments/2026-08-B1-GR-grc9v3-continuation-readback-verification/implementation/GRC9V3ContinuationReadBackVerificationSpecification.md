...
**Date:** 2026-08-21  
**Status:** Draft 3.4.1 — pre-execution mathematical and execution-sealing correction  
**Supersedes:** Draft 3.4  
...
substrate_base_revision = REQUIRED_BEFORE_EXECUTION
experiment_execution_revision = REQUIRED_BEFORE_EXECUTION
theory_revision = REQUIRED_BEFORE_EXECUTION
```
...
## 1.1 Draft 3.4.1 pre-execution mathematical and execution-sealing correction

Draft 3.4.1 preserves Draft 3.4's theory contract, gate sequence, claim
ceilings, unchanged-runtime boundary, and identification architecture. It
closes the remaining pre-execution correctness and governance ambiguities:

1. Floquet multipliers are derivatives of the admitted causal transition and
   are assigned only to cycles remaining within one continuous causal stratum;
2. the primary GRV6 cycle decomposition uses the native
   conductance-compatible metric $W_*^{-1}$, or another metric proved to
   annihilate native weighted potential flow in the cycle projection;
3. causal-state codec closure is tested over preregistered multi-step horizons,
   with explicit finite-difference stratum margins;
4. structural validity, constitutive consistency, runtime reachability, and
   runtime-causal independence receive operationally distinct admission rules;
5. linear susceptibility language requires a preregistered probe-amplitude
   sweep beyond the finite-probe difference-in-differences result;
6. result receipts consume accepted anchors as authorization, and acceptance
   authority and the present-current convention are preregistered;
7. later-discovered protected sources route through the contradiction register
   instead of silently mutating the accepted manifest;
8. experiment-tree, schema-coverage, theory-source, and closeout evidence-bundle
   scopes are complete and non-self-referential;
9. GRV0 package preparation and gate execution use separate clean revisions;
10. residual, positive-evidence, package-test, envelope-name mapping, and
    checklist emission requirements are propagated explicitly.

Draft 3.4.1 does not add a theory claim, alter a GRV gate, authorize a runtime
change, or open B1-L.

## 1.2 Draft 3.4 pre-execution identification and lifecycle hardening revision

Draft 3.4 preserves the complete Draft 3.3 theory and gate architecture while
closing second-order identification, codec, causal-inference, and evidence-
lifecycle ambiguities found before GRV0:

1. candidate gate results and scientific acceptance anchors are separate,
   non-self-referential records with explicit acceptance authority;
2. experiment-tree and protected-tree manifests have explicit scopes and
   versioning rules;
3. causal-state codecs must pass round-trip, reached-state canonicalization,
   and transition-commutation tests;
4. causally relevant discrete state is handled as a fixed stratum around a
   continuous Jacobian rather than coerced into a real coordinate;
5. counterfactual sensitivity, constitutive independence, runtime Markov
   necessity, and eliminability are separate decisions;
6. GRV5 uses a carrier-by-probe difference-in-differences contrast so baseline
   transport is not mislabeled as a read effect;
7. GRV2 nonuniform branches are bounded searches, and physical strong branches
   become causal strong branches only after GRV3 closure;
8. return orbits must close in admitted causal state and discrete stratum;
9. GRV6 freezes the edge metric and cycle/cut projectors before measuring cycle
   content;
10. schema/module surfaces and source-manifest evolution are explicit.

Draft 3.4 does not add theory claims, alter the GRV sequence, authorize runtime
changes, or open B1-L.

## 1.3 Draft 3.3 pre-execution implementation-hardening revision

Draft 3.3 preserves the theory contract, claim ledger, assumptions, debts,
fixtures, GRV gate sequence, unchanged-runtime boundary, and GRC-before-LGRC
order of Draft 3.2. The accepted Draft 3.2 source is retained beside this file
as `GRC9V3ContinuationReadBackVerificationSpecification_Draft3_2.md`.

This revision closes pre-execution ambiguities identified during implementation
audit without adding a theory claim or authorizing runtime changes:

1. the temporal eigensystem is assigned only to a square admitted causal-state
   transition, while smooth derived surfaces receive separate response
   Jacobians and categorical outputs remain threshold records;
2. conserved coherence perturbations use an explicit zero-sum tangent basis,
   and every coordinate block receives an interior-valid perturbation policy;
3. strong branches must distinguish constitutive fixedness from apparent
   fixedness produced by budget projection;
4. state interventions use a canonical clone/change/rebuild/validate protocol;
5. GRV5 separates full-step, immediate-stage, and frozen-conductance probes;
6. baseline identity distinguishes substrate, experiment, and theory revisions;
7. accepted gate receipts define the clean-input execution lifecycle;
8. semantic payload hashes are separated from volatile file metadata;
9. package, script, schema, experiment-test, preregistration, and numerical
   environment requirements are explicit;
10. the fixed-topology envelope, ambient identification, excluded-field audit,
    branch continuation, and closeout deliverables are propagated into the
    executable gates.

Draft 3.3 remains a pre-execution verification specification. It does not
authorize `src/` changes, existing-test changes, a read-back mechanism, an
LGRC probe, N32, L04, or `pygrc.analysis`.

## 1.4 Preserved Draft 3.2 propagation-audit basis
...
The Draft 3.2 basis did not turn candidate completions into core results,
select a unique retained projector or read-back operator, authorize
`pygrc.analysis`, select N32 or L04, or authorize substantive LGRC execution.
...
experiments/2026-08-B1-GR-grc9v3-continuation-readback-verification/
  ...
  implementation/
    GRC9V3ContinuationReadBackVerificationSpecification.md
    GRC9V3ContinuationReadBackVerificationSpecification_Draft3_2.md
    GRC9V3ContinuationReadBackVerificationSpecification_Draft3_3.md
    GRC9V3ContinuationReadBackVerificationSpecification_Draft3_4.md
    GRC9V3ContinuationReadBackVerificationImplementationPlan.md
    GRC9V3ContinuationReadBackVerificationImplementationChecklist.md
  configs/
    baseline.json
    ...
    branch_search.json
    orbit_search.json
    nonnormal_control.json
    fast_slow_control.json
    persistence_protocol.json
  fixtures/
  ...
  scripts/
    ...
    run_all.py
  schemas/
    common_artifact.schema.json
    gate_result_receipt.schema.json
    gate_acceptance.schema.json
    baseline_manifest.schema.json
    state_codec.schema.json
    fixed_branch_registry.schema.json
    complete_step_jacobians.schema.json
    intervention_registry.schema.json
    causal_role_matrix.schema.json
    return_orbit_registry.schema.json
    evidence_bundle_manifest.schema.json
    assumption_status_matrix.schema.json
  ...
  tests/
    ...
    test_state_codec.py
    test_tangent_basis.py
    test_gate_receipts.py
    test_numerical_convergence.py
    test_edge_space.py
...
The `B1-GR` directory qualifier and the `ImplementationPlan` /
`ImplementationChecklist` names are the accepted repository path corrections
to Draft 3.2. They distinguish the synchronous GRC verification from deferred
`B1-L` and match existing repository naming. All other names remain normative
unless a later correction is recorded explicitly. No numbered `N32` identity
is assigned.

The `hypotheses/` files are serialized human-readable views of the controlling
claim, assumption, derivation, debt, traceability, and gate records. They do not
constitute separate positive hypotheses or evidence.

The experiment-local numerical requirements file records only libraries used
by B1-GR analysis. It must not alter runtime package dependencies. If existing
declared dependencies suffice, the file records that fact without adding a
package.

At GRV0, dedicated schemas or named `$defs` in
`common_artifact.schema.json` must cover at least the protected-path manifest,
experiment-path manifest, theory-source manifest, numerical environment,
evidence-bundle manifest, contradiction register, theory-reopening decision,
and extension decision in addition to the dedicated package schemas above.
Schema coverage is validated mechanically; prose fields alone are not package
admission.
...
```json
{
  "schema_version": "b1_grc9v3_verification_v3_4_1",
  "specification_id": "b1_grc9v3_continuation_readback_verification_v3_4_1",
  "repository": "github.com/urosj/graph-reflexive-coherence",
  "substrate_base_revision": "<exact Git commit SHA>",
  "experiment_execution_revision": "<clean input experiment sha>",
  "branch_name": "<informational only>",
  "dirty": false,
  "theory_repository": "github.com/urosj/geometric-reflexive-coherence",
  "theory_revision": "<exact sha>",
  "protected_path_manifest_v0_path": "outputs/protected_path_manifest_v0.json",
  "protected_path_manifest_v1_expected_path": "outputs/protected_path_manifest_v1.json",
  "protected_tree_sha256": "<semantic tree digest>",
  "experiment_tree_sha256": "<semantic experiment tree digest>",
  "experiment_path_manifest_path": "outputs/experiment_path_manifest.json",
  "theory_source_manifest_path": "outputs/theory_source_manifest.json",
  ...
  "test_counts": {},
  "test_log_path": "<relative path>",
  "created_at_utc": "<ISO-8601>",
  "runtime_change_authorized": false
}
```

`substrate_base_revision` is the exact Git commit SHA. It is not interchangeable
with `protected_tree_sha256`, which is the content digest of a selected path
set.

`protected_path_manifest_v0` must enumerate every load-bearing file and blob or
SHA-256 digest under the GRC source, specification, and test surfaces admitted
at GRV0. GRV1 may produce `protected_path_manifest_v1` only to add newly
discovered load-bearing paths. Every added path must match the frozen substrate
base revision. If GRV1 finds no additional path, v1 must still be emitted as an
explicit unchanged successor to v0. GRV2 and later gates consume the accepted
v1 digest.

If GRV2 or a later gate discovers another load-bearing protected path, v1 is
not edited silently. The discovery creates a contradiction-register entry with
route `source_or_specification_mismatch`, blocks dependent interpretation, and
requires an explicit revision-distinct baseline readmission decision.

`experiment_tree_sha256` covers every committed file under the experiment
directory except the explicitly excluded surfaces:
...
This includes the root `README.md`, implementation documents, hypotheses,
configs, fixtures, scripts, schemas, tests, requirements, and any later
committed non-output control file. `experiment_path_manifest_path` records the
exact included files. The digest is gate-relative: every result receipt records
the clean input revision and experiment-tree digest actually used by that gate.

`outputs/theory_source_manifest.json` records the exact theory revision and the
repository-relative path, source role, blob identity where available, and
SHA-256 digest of each controlling core paper, including *The Continuation
Spectrum* and *Read-Back*.

## 14.4 Clean-input gate lifecycle

Clean checkout is an input condition, not a claim that a completed gate creates
no outputs. The initial documentation scaffold is committed first. Before GRV0
execution, a separate package-preparation revision `P0` materializes all named
package surfaces, completes the GRV0 executables and tests, and identifies
later-gate surfaces that are not yet executable. GRV0 then runs from clean
`P0`; it does not create or execute uncommitted experiment code.

Before each later gate, any gate-local implementation completion or correction
is committed in a new clean input revision. Its experiment-path manifest and
tree digest are recorded by that gate. Gate execution produces evidence and
reports, not source code. A later experiment-code revision cannot mutate an
already accepted result or change the protected substrate revision.
...
clean accepted input revision
  ...
  -> write acceptance anchor referring to R_n
  -> commit or otherwise freeze acceptance anchor A_n
  -> next gate consumes A_n
```

Each `outputs/gates/grvN_result_receipt.json` records:

```text
gate_id
input_execution_revision
substrate_base_revision
input_experiment_tree_sha256
...
Only `prerequisite_acceptance_anchors` authorizes progression. Receipt digests
establish result provenance but not scientific acceptance. GRV0 records empty
prerequisite lists.

The receipt excludes itself from `output_artifact_digests` and cannot contain
the SHA of the commit that will contain it. Candidate receipt status is one of:
...
Scientific acceptance is a separate
`outputs/gates/grvN_acceptance_anchor.json` containing:
...
acceptance_status = accepted | rejected | blocked | superseded
acceptance_signature_or_ref
```

The anchor does not contain its own future commit SHA. The next gate records
the anchor payload digest and the already-existing commit or immutable ref that
contains it. A subsequent small commit is the default acceptance mechanism;
signed tags, Git notes, or another explicitly frozen sidecar are allowed when
their authority and retrieval rule are declared.

Before `P0`, `configs/preregistration.json` freezes:

```text
authorized_acceptance_roles
authorized_acceptors_or_identity_rule
required_review_method_per_gate
self_review_permitted
independent_review_required
```

Permitted human roles are `experiment_owner`, `designated_scientific_reviewer`,
and `independent_reviewer`; a repository-specific equivalent may be declared in
the preregistration. Mechanical scripts and `run_all.py` are never accepting
authorities. GRV0 is bootstrap-trusted only in the narrow sense that its
preregistered human authority reviews the first mechanically validated result;
it is not exempt from the authority record or review method.

`run_all.py` may orchestrate eligible gates but must refuse to cross a missing,
rejected, blocked, or superseded prerequisite acceptance anchor. Mechanical
validation cannot manufacture scientific acceptance.
...
- and include a SHA-256 digest of the canonical semantic payload.
...
Machine artifacts use a non-self-referential envelope:
...
```json
{
  ...
  "metadata": {},
  "payload": {},
  "payload_sha256": "<sha256 of canonical payload only>"
}
```

Volatile metadata such as generation time is excluded from `payload_sha256`.
Where the complete file is hashed, `artifact_file_sha256` is recorded outside
the file or in its manifest. Each artifact declares one reproducibility class:

```text
byte_reproducible
tolerance_reproducible
search_reproducible_with_seed_and_budget
```

GRV0 must also emit `outputs/numerical_environment.json` with the Python
executable/version, dependency versions, OS/architecture, BLAS/LAPACK provider,
thread controls, `PYTHONHASHSEED`, locale, and floating-point information.
...
GRV0 must serialize these values as the fixed-topology envelope rather than
leaving them implicit in prose. Every primary perturbation row must verify the
envelope before its result is admitted.

The serialized envelope must include an audited mapping from every
specification-side name, including `lambda_birth`, `boundary_action_required`,
and `quadrature_mode`, to the exact `GRCParams`, constructor, or runtime field
that enforces it. An unmapped or mismatched name blocks the envelope rather
than being treated as a silently applied parameter.
...
GRV0 must freeze an orthonormal coherence tangent basis

$$
Q_C\in\mathbb R^{|V|\times(|V|-1)},
\qquad
Q_C^TQ_C=I,
\qquad
\mathbf 1^TQ_C=0,
...
for each topology/order family, plus the fixed-topology ambient coordinate
identification and any branch-dependent inner-product transport. GRV3 must use
that basis rather than perturbing unconstrained node-coordinate unit vectors.
...
## 18.2 Causal-state coordinate and square transition

The physical projection in Section 18.1 is an observation bundle. It is not
automatically the coordinate domain of one eigensystem because it includes
derived surfaces and categorical outputs.

GRV3 must first admit a branch-relative continuous causal-state encoder

$$
E_X:\text{model state}\rightarrow\mathbb R^n
$$

and a valid-state decoder/intervention map

$$
D_X:\mathbb R^n\rightarrow\text{model state}.
$$

The square causal transition is

$$
F_X=E_X\circ F\circ D_X,
$$
...
receives temporal eigenvalues, invariant subspaces, Riesz projectors,
participation ratios, or transition multipliers.

The initial candidate coordinate is

$$
x=(C,W,J),
$$

augmented by any field that the GRV1/GRV3 closure audit classifies as causal or
unknown and that is not an exact deterministic reconstruction of admitted
coordinates. GRV3 must determine rather than assume whether this state is
sufficient.

Candidate reductions are:

```text
admitted complete continuous causal coordinate
(C,W,J) nonlinear state if closure passes
(C,W) quiescent tangent state if the J block is eliminable
C-only tangent state only if the W block is also eliminable on that branch
```

Before $F_X$ is admitted, `test_state_codec.py` and GRV3-A must establish:
...
for every admitted coordinate state within the preregistered tolerance.

### Reached-state canonicalization

$$
D_X(E_X(S))\sim_{\mathrm{causal}}S
$$

for reached runtime states. Byte equality is unnecessary for excluded observer
caches, but accepted causal-state equivalence is mandatory.

### Transition commutation

$$
E_X(F(S))\approx F_X(E_X(S)).
$$

GRV3-A must also test bounded-horizon closure over a preregistered set
$\mathcal H_{\mathrm{codec}}$, defaulting to complete-beat horizons
$\{1,2,5,10\}$ unless a fixture-specific replacement is justified:

$$
E_X(F^k(S))\approx F_X^k(E_X(S)),
\qquad
k\in\mathcal H_{\mathrm{codec}}.
$$

Passing supports `causal_closure_verified_through_declared_horizon`, not an
unbounded Markov-sufficiency claim. Exact reconstructibility or causal
inertness may support a stronger field-specific exclusion only when established
independently from source semantics and controls.

Failure of any codec test blocks temporal eigenanalysis because the encoded map
would not yet be demonstrated as the runtime transition.

If the closure audit finds a causally relevant discrete field, the admitted
state is stratified as $(\sigma,x)$: $\sigma$ is the fixed discrete stratum and
$x$ the continuous coordinate. $DF_{X,\sigma}$ is computed only inside one
stratum. Transitions changing $\sigma$ are categorical records, not real-valued
Jacobian coordinates.

Every finite-difference column must record a positive stratum margin and verify
that both $x_*+h_i e_i$ and $x_*-h_i e_i$ decode into the same $\sigma$, remain
in that stratum through the complete step, stay outside every declared
categorical threshold margin, and follow the same load-bearing runtime path.
Failure means the within-stratum classical derivative is undefined at that
step size; the column is blocked or routed to categorical/one-sided threshold
evidence rather than called merely unconverged.

For `W`, `J`, and any additional candidate surface, GRV3 assigns these decisions
separately:

```text
counterfactual_sensitive:
  derivative is nonzero under a structurally valid intervention

constitutively_independent:
  branch-consistent variation exists at fixed smaller coordinates

runtime_causal_independent:
  reached or accepted states matched in smaller coordinates but differing in
  the candidate surface produce different futures

eliminable:
  the surface is uniquely reconstructed from smaller accepted state and the
  reduced transition commutes with the complete transition
```

Counterfactual sensitivity alone does not establish an independently necessary
runtime coordinate.

For every smooth derived surface $Y$, compute a separate response Jacobian

$$
R_Y=D(E_Y\circ F\circ D_X)(x_*).
$$

Examples include $R_\Phi$, $R_G$, $R_{H_s}$, and $R_K$. These explain how
causal perturbations appear on derived surfaces; they do not enlarge the square
state transition unless the closure audit independently admits them as state.

Sinks, basins, hierarchy, spark eligibility, event classes, and discontinuous
budget records are handled through branch-margin checks, one-sided threshold
evidence, and categorical transition records. They are not inserted as rows or
columns in the temporal eigensystem.

GRV3 is implemented internally as:

```text
GRV3-A = causal-state closure and continuous coordinate admission
GRV3-B = square complete-step Jacobian on admitted coordinates
GRV3-C = smooth response Jacobians and categorical threshold surfaces
```
...
## 18.5 Canonical state intervention protocol

Every direct intervention must start from an exact deep clone. The live object
returned by `get_state()` must not be mutated in place. The experiment must use
`set_state()`, `from_state()`, or an equivalently explicit clone path and then
apply one declared rebuild policy.

`scripts/interventions.py` and `outputs/intervention_registry.json` define and
record at least:
...
fields directly changed
fields explicitly held fixed
fields rebuilt afterward
rebuild order
validity checks
reachability status
physical projection before/after
causal-state projection before/after
```

Canonical operations include:

```text
set_coherence_tangent
set_base_conductance
set_old_current
reverse_physical_current
reorient_edge_coordinate
reset_carrier
swap_carrier
match_C_and_J
```

Each synthetic intervention is classified independently as
`structurally_valid`, `constitutively_consistent`, and `runtime_reachable`.
Duplicated conductance/current fields and derived caches must be reconciled by
the declared rebuild order; a hybrid partially updated state is inadmissible.

The admission statuses mean:

| Status | Minimum evidence |
|---|---|
| `structurally_valid` | serialization, positivity, budget, topology, duplicate-field reconciliation, and runtime validation pass |
| `constitutively_consistent` | relevant reconstruction and branch residuals pass, or a declared branch-constrained solver produces the state |
| `runtime_reachable` | unchanged runtime reaches the state from an accepted initial state under a recorded intervention/history |
| `runtime_causal_independent` | reached or accepted constitutively consistent states, matched on the proposed smaller state but differing in the candidate coordinate, produce different futures |

A structurally valid off-manifold intervention may establish counterfactual
sensitivity. It cannot by itself establish constitutive or runtime-causal
independence. GRV5 rows based only on such synthetic states remain
`synthetic_counterfactual` and cannot reach the same causal ceiling as a result
reproduced on reached or constitutively consistent states.
...
L_2 residual
relative residual
per-block residual for C, W, J, Phi, G, identity, and budget correction/no-op
```

The staged replay must record:

```text
pre-continuity state
post-continuity / pre-budget state
budget correction vector
post-budget state
final-refresh state
```

Accepted branch classes are:

```text
provisional_physical_strong_branch
causal_strong_branch
projection_supported_fixed_point
step_boundary_only_fixed_point
internal_periodic_state
```

GRV2 may assign `provisional_physical_strong_branch` when the declared physical
and internal residuals pass and budget correction is a numerical no-op. GRV3
may upgrade it to `causal_strong_branch` only after excluded causal fields are
stationary, reconstructible, or irrelevant under the accepted closure audit.
Other classes remain evidence with their projection or periodic qualification.
The internal-stage trace is produced by staged re-execution on a fresh clone,
not by patching callbacks into `step()`.
...
Every candidate records its distance from zero coherence, the conductance floor,
spark/event thresholds, basin/sink transitions, and growth eligibility. A
central derivative is not admitted across a non-smooth boundary.

## 19.3 Complete-step causal transition and response Jacobians

The temporal Jacobian is the square admitted causal-state transition from
Section 18.2:

$$
A_*=D(E_X\circ F\circ D_X)(x_*).
$$

Use reduced coherence coordinates

$$
C=C_*+Q_Ca
$$

and an admitted tangent coordinate

$$
\xi=(a,\omega,q,\ldots),
$$

where $a$ is the zero-sum coherence coordinate, $\omega$ is the admitted
conductance coordinate, $q$ is the admitted current coordinate, and any further
block has passed the causal-state closure audit.

Conductance uses either an interior-safe additive coordinate with a proved
margin or the log coordinate

$$
\omega_e=\log W_e.
$$

Each block declares a relative scale, absolute perturbation floor, maximum
valid perturbation, and interior-margin test. In particular, a zero branch
coordinate does not receive a zero step. A default form is

$$
h_i=\epsilon\max(|\xi_i|,s_i^{\mathrm{abs}}),
$$

subject to the stricter valid-state margin.

Use central differences on valid tangent perturbations:

$$
A_*e_i
\approx
\frac{F_X(\xi_*+h_ie_i)-F_X(\xi_*-h_ie_i)}{2h_i}.
$$

Smooth response Jacobians use the same admitted input coordinates but their
own declared output projection. Categorical outputs use threshold/margin
records rather than central-difference eigensystems.
...
The selected derivative must lie in a convergence region and include a
truncation estimate. GRV0 preregisters relative column error, adjacent-step
matrix-norm error, eigenvalue-cluster distance, and subspace principal-angle
thresholds. Near degeneracy, eigenvalues are matched as clusters rather than
sorted by index.
...
A period-$p$ orbit must return in the admitted causal state:

$$
\|E_X(F^p(S))-E_X(S)\|\le\varepsilon_{\mathrm{orbit}},
$$

while

$$
\|E_X(F^q(S))-E_X(S)\|>\varepsilon_{\mathrm{primitive}}
$$

for all proper divisors $q<p$.

It must also preserve equality of causally relevant categorical event state
and show only expected administrative advancement. A trajectory returning only
under the physical projection is classified `physical_projection_return`, not
a full runtime return orbit.

Ordinary Floquet multipliers are admitted only when the orbit remains in one
admitted continuous causal stratum $\sigma$ at every intermediate point and
every derivative probe stays within that stratum. With
$x_k=E_X(F^k(S))$, define
...
D F_{X,\sigma}(x_k)
=
D(E_X\circ F\circ D_X)\big|_{x_k,\sigma},
$$

and the causal return-map derivative is

$$
\mathcal M_p=A_{p-1}\cdots A_1A_0.
$$

An orbit crossing discrete strata is recorded as
`hybrid_or_categorical_return_orbit` without a Floquet spectrum. It may receive
a hybrid spectrum only in a later analysis that explicitly defines and
validates every inter-stratum transition/reset derivative, such as a saltation
map, and preregisters their composition. The ambiguous full-state expression
$D(F^p)$ is not used as a substitute for the admitted causal derivative.
...
The generic `positive_evidence_opened` flag remains false through GRV0 and
GRV1. It may become true only when GRV2 or a later gate accepts at least one
source-current scientific row. Role-specific continuation, retention, read,
write, and recurrence flags remain separate and change only at their own gates.
An accepted instrumentation or schema row never opens positive evidence.
...
- clean package-preparation revision committed before gate execution;
...
- normative `hypotheses/` views and named script-to-gate map created;
- artifact, result-receipt, acceptance-anchor, baseline, manifest, branch,
  Jacobian, intervention, cycle/return, decision, and evidence-bundle schema
  coverage created;
- experiment-local schema, codec, tangent, intervention, receipt, edge-space,
  and numerical tests created;
- numerical dependency and environment policy frozen;
- `A-NONNORMAL-CONTROL` evidence mode/threshold preregistered;
- `A-FAST-SLOW` measure/threshold preregistered where applicable;
- present-current convention and acceptance authority/review policy preregistered;
- fixed-topology envelope, tangent bases, ambient coordinate identification,
  and any branch-dependent inner-product transport frozen;
- specification-name to runtime-parameter envelope mapping validated;
...
After rebuilding differential state, clone two states that agree in all physical load-bearing fields but differ strongly in cached `hybrid_node_tensors`.

Run two separate lanes:

```text
K-transport-lane:
  perturb K immediately before transport reconstruction

K-full-step-lane:
  perturb K before a complete step and test whether the differential stage
  overwrites it before any downstream use
```

The equality comparison excludes the intentionally changed cache itself and
tests load-bearing transport and later physical outputs.
...
Physical reversal and coordinate reorientation are distinct:

```text
physical current reversal:
  fixed edge coordinates, J -> -J

edge-coordinate reorientation:
  reverse the canonical edge coordinate and map J -> -J
```

The first tests sign sensitivity; the second tests orientation covariance.

### GRV1-E — excluded and administrative field inventory

Inventory every field excluded from the physical projection or advanced for
administrative reasons. Classify each as deterministic administrative,
causal runtime, observer-only, reconstructed, or unknown. Causal and unknown
fields remain in the GRV3 closure-state candidates.
...
Failure to find a nonuniform branch within the preregistered search seeds,
parameter envelope, and compute budget is bounded negative search evidence. It
does not fail GRV2 when at least one required homogeneous strong branch passes,
and it does not establish global nonexistence.
...
pre/post continuity and budget states
budget correction vector and no-op status
branch class
distance from non-smooth boundaries
GRV3 causal-branch upgrade status
```
...
GRV3-A admits the continuous causal-state coordinate, GRV3-B computes the
square transition Jacobian, and GRV3-C computes smooth response Jacobians and
categorical threshold records.
...
```text
causal-state encoder/decoder identities
admitted coordinate order and tangent basis
square causal-transition matrix
smooth derived response matrices
categorical threshold surface records
C/W/J blocks
eigenvalues
...
coordinate scales, absolute floors, maximum steps, and interior margins
column, matrix, eigenvalue-cluster, and subspace convergence
```

## Decision rule

- If $D_WF\neq0$, the transition is `W`-sensitive in the admitted decoder
  chart. Independent-state status requires separate constitutive or reached-state
  evidence.
- If $D_JF=0$ at $J=0$, old current is absent as an independent first-order
  sensitivity on that branch; broader runtime-state necessity remains subject
  to the closure and elimination tests.
...
- Eigenanalysis is blocked until the admitted causal transition is square and
  its state codec, tangent basis, and convergence checks pass.
...
Report separately the analytic semidiscrete sign, finite-step monotonicity at
the implemented timestep, and a preregistered timestep sweep. A continuous
monotonicity identity does not establish discrete monotonicity at arbitrary
timestep. The comparator uses the same conserved tangent basis as GRV3.
...
Before any GRV5 outcome is observed, `configs/preregistration.json` must freeze
the present-current convention used by `A-PASSIVE`, including what counts as a
present-current input, an old-current state intervention, a
coherence/potential probe, and an external analytical probe. Under the frozen
source contract, a coherence-driven change in executed current is not silently
reclassified as an external present-current input. A GRV1 source contradiction
must be routed explicitly rather than changing this convention after outcomes.
...
The probe must be executed and classified in three separate lanes:

```text
native_full_step_probe:
  execute the complete unchanged recurrence

native_immediate_transport_stage_probe:
  execute the exact public stage sequence through first transport
  reconstruction on a clone

frozen_W_probe:
  hold W fixed while computing potential/current; substrate_reduced only
```

The readout stage and causal path must be explicit for every lane. Results from
the frozen-`W` lane cannot be reported as native full-step response.

The probe input must also be classified as exactly one of:
...
Unchanged GRC does not expose a separate external present-current input. A
coherence perturbation driving potential-conditioned transport is not by
itself a present-current-conditioned read-back probe. An external-current-like
probe is analytical rather than executed substrate mechanics.

### Carrier-by-probe response contrast

Every candidate read-effect row must execute the full $2\times2$ contrast for
carrier states $W_a,W_b$ and probe states $p,0$:

$$
\Delta J_a=J(W_a,p)-J(W_a,0),
$$

$$
\Delta J_b=J(W_b,p)-J(W_b,0),
$$

$$
\Delta_{\mathrm{carrier}\times\mathrm{probe}}
=\Delta J_a-\Delta J_b.
$$

This difference-in-differences term identifies whether the carrier changes the
incremental response to the present probe rather than merely changing baseline
geometry-conditioned current.

Claim routing is:

```text
different no-probe baselines but equal incremental response:
  ordinary geometry-conditioned baseline recurrence

different incremental response under coherence/potential probe:
  carrier-conditioned susceptibility or transport-response candidate

different response only under old-current state injection:
  current recurrence or write/read geometry candidate, not automatic Read-Back

different response only under external-current-like analytical probe:
  analysis_only, not substrate_exact
```

The contrast, baseline terms, incremental terms, uncertainty/tolerance, and
lane-specific claim ceiling must be recorded explicitly.

A finite-probe interaction does not by itself establish a linear
susceptibility, gain, or derivative. Any row using those terms must execute a
preregistered signed amplitude sweep, for example
$p\in\{-2\epsilon,-\epsilon,\epsilon,2\epsilon\}$, verify local convergence or
linearity within its declared tolerance, and report the residual from the
fitted first-order response. Without that sweep, the maximum wording is
`finite_probe_carrier_by_probe_interaction`.
...
The mediation component of `GRR4` or `GRR5` requires at least a
constitutively consistent candidate state under the operational gate in
Section 18.5. A structurally valid off-manifold synthetic intervention may
support a counterfactual interaction diagnostic, but cannot satisfy those
causal mediation rungs by itself. Runtime-reached reproduction is recorded as
the stronger provenance class.
...
## 6.0 Edge metric and Hodge-decomposition contract

Cycle-content claims require a preregistered edge-space geometry. For every
GRV6 topology, freeze:

```text
oriented edge order
node-by-edge incidence convention B
positive-definite edge metric M
edge inner product <u,v>_M = u^T M v
cycle-space projector P_cycle
potential-complement projector P_potential = I - P_cycle
divergence residual norm and tolerance for B J = 0
edge-reorientation covariance rule
metric policy across branch/orbit states
```

On a positive-conductance branch, the primary GRV6 metric is the native
constitutive metric
...
where $W_*$ is the positive diagonal edge-conductance operator in the declared
edge order. Another primary metric is admissible only with a proof and numeric
control that the native potential-flow image
$W_*\operatorname{range}(B^T)$ lies in its declared non-cycle complement.
Euclidean, conductance-weighted, or other metrics may be reported as sensitivity
analyses but cannot define the primary cycle-retention claim merely because
they produce a preferred decomposition.

For a full-column-rank basis $Z$ of $\ker B$, the declared metric-orthogonal
cycle projector is

$$
P_{\mathrm{cycle},*}
=
Z\left(Z^T W_*^{-1} Z\right)^{-1}Z^T W_*^{-1},
$$

with

$$
P_{\mathrm{potential},*}=I-P_{\mathrm{cycle},*}.
$$

This complement is the $W_*^{-1}$-orthogonal potential/cut complement. It is
called the ordinary cut space only if equality with
$\operatorname{range}(B^T)$ has been proved under the declared coordinates.
The primary constitutive control must verify

$$
P_{\mathrm{cycle},*}\left(-\eta W_*B^T\Phi\right)=0
$$

within tolerance for admitted native potential-flow currents.

Numerical implementations may use an equivalent rank-revealing
factorization, but must report rank tolerance, projector idempotence residual,
metric-orthogonality residual, and decomposition reconstruction residual.

`configs/cycle_space.json` must state whether the metric/projector is frozen at
a reference branch, recomputed from $W_k$ at each orbit point, or compared
through a declared metric transport/normalization. A frozen reference metric
may be used away from its reference state only when the native potential-flow
annihilation control still passes. Recomputed projectors require the declared
transport before component magnitudes are compared across orbit points.

For an edge-orientation change represented by a diagonal sign matrix $R$,
require
...
$$
P'_{\mathrm{cycle}}=R P_{\mathrm{cycle}}R,
\qquad
P'_{\mathrm{cycle}}J'=R P_{\mathrm{cycle}}J
$$

within the declared tolerance. A cycle-space measurement that changes under a
pure coordinate reorientation is invalid.
...
Before reconstruction, certify

$$
B J_{\mathrm{cycle}}=0
$$

under the declared incidence and edge-orientation convention. Record initial
cycle-space content, conductance inscription caused by the seed, reconstructed
current, and remaining cycle-space projection separately.
...
Follow each branch through continuation rather than solving unrelated branch
points independently. Preregister the branch matching invariant, cluster
matching method, maximum parameter step, and bifurcation stop/restart rule.
...
outputs/gates/grv0_result_receipt.json through outputs/gates/grv8_result_receipt.json
outputs/gates/grv0_acceptance_anchor.json through outputs/gates/grv8_acceptance_anchor.json
...
GRV-C6 = claim classification, contradiction routing, theory-reopening decision,
extension decisions, accepted evidence bundle, evidence-grounded successor,
GRV8 closeout anchor, and LGRC handoff complete
```
...
```json
{
  "schema_version": "b1_grc_to_lgrc_handoff_v3_4_1",
  "grc_substrate_base_revision": "<sha>",
  "b1_gr_experiment_execution_revision": "<sha>",
  "theory_revision": "<sha>",
...
## 31.1 Non-self-referential evidence bundle and successor order

GRV8 first produces and scientifically accepts its classification result through
the ordinary `grv8_result_receipt.json` and `grv8_acceptance_anchor.json`.
It then freezes `outputs/evidence_bundle_manifest.json`. The manifest payload
lists accepted GRV0-GRV8 result revisions, acceptance-anchor digests, accepted
scientific artifacts and reports, and final assumption/claim matrices.

The evidence-bundle payload excludes:

```text
outputs/evidence_bundle_manifest.json itself
the evidence-grounded successor specification
any manifest whose subject is that successor
the later GRV8 closeout acceptance anchor
```

The order is:

```text
accepted GRV8 scientific classification
  -> evidence-bundle manifest and payload digest
  -> evidence-grounded successor referencing that digest
  -> outputs/gates/grv8_closeout_acceptance_anchor.json
  -> GRV-C6 assignment
```

The closeout anchor records the evidence-bundle digest, successor digest,
accepted GRV8 anchor digest, accepting authority, review method, and immutable
reference. It does not alter the evidence-bundle payload retroactively.

Before GRV-C6, the experiment must produce:

- the accepted non-self-referential evidence-bundle manifest and the GRV8
  closeout acceptance anchor defined in Section 31.1;
- an evidence-grounded successor specification at
  `implementation/GRC9V3ContinuationReadBackVerificationSpecification_EvidenceGrounded_v1.md`;
- the predecessor specification ID/digest, accepted evidence-bundle digest,
  changed claim/assumption/debt rows, and superseded exploratory statements in
  that successor, while preserving Drafts 3.2, 3.3, and 3.4 and the accepted
  controlling Draft 3.4.1 unchanged;
...
- a final complete existing-suite result from the GRV8 clean input revision;
...
- [ ] Materialize the normative hypotheses views, script map, schemas, configs, and experiment-local tests.
- [ ] Freeze semantic-payload hashing and the separate result-receipt /
  authority-bearing acceptance-anchor lifecycle.

## Repository baseline

- [ ] Record substrate-base and experiment-execution revisions separately.
- [ ] Commit a clean GRV0 package-preparation revision before executing GRV0.
- [ ] Confirm clean checkout.
- [ ] Run and record the complete existing test suite.
- [ ] Hash dependency and configuration files.
- [ ] Freeze protected-path manifest v0, experiment-path manifest, and
  theory-source manifest; freeze the GRV1-only v1 expansion rule.
- [ ] Record both controlling paper digests in the theory-source manifest.
- [ ] Record the complete numerical environment and analysis dependency policy.
...
- [ ] Freeze the causal-state encoder/decoder, zero-sum tangent basis, and interior-safe coordinate policy.
- [ ] Require coordinate round trip, reached-state causal canonicalization,
  one- and multi-step transition commutation, stratum margins, and fixed-stratum
  treatment of categorical state.
- [ ] Freeze operational structural-validity, constitutive-consistency,
  reachability, and runtime-causal-independence admission rules.
- [ ] Freeze the canonical clone/intervention/rebuild protocol.
- [ ] Freeze JSON canonicalization and digests.
- [ ] Freeze numerical tolerances before scientific execution.
- [ ] Validate specification-name to runtime-parameter mappings for the fixed-topology envelope.
- [ ] Verify no event or topology change across all primary perturbations.

## Scientific gates

- [ ] Certify homogeneous strong branches.
- [ ] Search for nonuniform strong branches, certify accepted results, and
  preserve bounded negative search evidence when none are found.
- [ ] Admit a square causal-state transition before temporal eigenanalysis.
- [ ] Compute complete-step causal and smooth response Jacobians separately with convergence checks.
- [ ] Keep categorical/event surfaces outside the differentiable eigensystem.
...
- [ ] Run carrier-by-probe difference-in-differences for every candidate read row.
- [ ] Run a preregistered amplitude sweep before using linear susceptibility/gain language.
- [ ] Freeze the native conductance-compatible GRV6 edge metric,
  cycle/potential projectors, metric policy, and orientation-covariance checks.
- [ ] Search and classify return orbits.
- [ ] Require return-orbit closure in admitted causal state and discrete stratum.
...
- [ ] Produce the versioned evidence-grounded successor while preserving Drafts
  3.2, 3.3, and 3.4 and the accepted controlling Draft 3.4.1.
- [ ] Freeze the accepted evidence-bundle manifest before generating the successor.
- [ ] Accept the successor and bundle through the separate GRV8 closeout anchor.
- [ ] Emit the superseded-exploratory-claims register.
- [ ] Rerun and record the complete existing suite at GRV8.
- [ ] Validate all GRV0-GRV8 result receipts and acceptance anchors and freeze
  the unchanged-GRC evidence bundle.
