...
**Date:** 2026-08-21  
**Status:** Draft 3.4 — pre-execution identification and lifecycle hardening revision  
**Supersedes:** Draft 3.3  
...
```text
specification_id = b1_grc9v3_continuation_readback_verification_v3_4
...
## 1.1 Draft 3.4 pre-execution identification and lifecycle hardening revision
...
## 1.2 Draft 3.3 pre-execution implementation-hardening revision
...
## 1.3 Preserved Draft 3.2 propagation-audit basis
...
```json
{
  "schema_version": "b1_grc9v3_verification_v3_4",
  "specification_id": "b1_grc9v3_continuation_readback_verification_v3_4",
...
`experiment_tree_sha256` covers committed experiment inputs under:
...
It excludes `outputs/`, `reports/`, and the manifest computing the digest.
`experiment_path_manifest_path` records the exact included files.

## 14.4 Clean-input gate lifecycle

Clean checkout is an input condition, not a claim that a completed gate creates
no outputs. Each gate uses:
...
It must also return to the same discrete causal stratum, preserve equality of
causally relevant categorical event state, and show only expected
administrative advancement. A trajectory returning only under the physical
projection is classified `physical_projection_return`, not a full runtime
return orbit.
...
- artifact, gate-receipt, baseline, branch, Jacobian, and intervention schemas created;
- experiment-local schema, codec, tangent, intervention, receipt, and numerical tests created;
...
cut-space projector P_cut = I - P_cycle
divergence residual norm and tolerance for B J = 0
edge-reorientation covariance rule
```

The metric must be named as Euclidean, conductance-weighted,
inverse-conductance-weighted, or another explicitly justified positive metric.
It cannot be selected after observing which decomposition gives the preferred
cycle content.
...
P_{\mathrm{cycle}}
=
Z\left(Z^T M Z\right)^{-1}Z^T M,
$$

with

$$
P_{\mathrm{cut}}=I-P_{\mathrm{cycle}}.
$$
...
```json
{
  "schema_version": "b1_grc_to_lgrc_handoff_v3_4",
...
- the predecessor specification ID/digest, accepted evidence-bundle digest,
  ...
  that successor, while preserving Draft 3.2, Draft 3.3, and the accepted
  controlling Draft 3.4 unchanged;
...
- [ ] Require coordinate round trip, reached-state causal canonicalization,
  transition commutation, and fixed-stratum treatment of categorical state.
...
- [ ] Freeze the GRV6 edge metric, cycle/cut projectors, and orientation-covariance checks.
...
- [ ] Produce the versioned evidence-grounded successor while preserving Drafts
  3.2, 3.3, and the accepted controlling Draft 3.4.
